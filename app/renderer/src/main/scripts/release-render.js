...
try {
    ...
    // 生成更新版本号分支
    execSync(`git checkout -b update/render-${version} origin/master`, {stdio: "inherit"})

    console.log("\x1b[32m%s\x1b[0m", `### Creating render branch: update/render-${version} ###`)
    ...
    console.log("\x1b[32m%s\x1b[0m", `### Creating render tag: v${version}-render ###`)
} catch (err) {
    console.error("生成 render 版本号命令失败:", err)
    process.exit(1)
}
