...
import {YakitCEMode} from "@/store/menuMode"
...
/** 通过版本获取一级tab固定展示tab  */
export const getDefaultFixedTabs = (menuMode: YakitCEMode | undefined) => {
    if (isMemfit()) {
    ...
    if (isYakit()) {
        ...
        if (isCommunityYakit()) {
            ...
            } else if (menuMode === "classic" || menuMode === "scan") {
                return [YakitRoute.NewHome, YakitRoute.DB_HTTPHistory]
            }
            return []
        }
...
/** 一级tab固定展示tab支持多开页面 */
export const getDefaultFixedTabsNoSinglPageRoute = (menuMode: YakitCEMode | undefined) => {
    if (isYakit()) {
        if (isEnpriTraceAgent()) {
            return []
        }
 ...
 */
export const getPublicRouteMenu = (menuMode?: YakitCEMode) => {
    if (isIRify()) {
        return [
            {
                page: undefined,
    ...
    return [
        {
            page: YakitRoute.MITMHacker,
            i18n: false,
            label: i18n.language === "en" ? "Open MITM" : "开启MITM"
        },
        {
            page: YakitRoute.HTTPFuzzer,
            ...YakitRouteToPageInfo[YakitRoute.HTTPFuzzer]
...
/** @name 右侧额外菜单 */
export const getExtraMenu: (menuMode?: YakitCEMode) => ExtraMenuItem[] = (menuMode) => {
    if (isIRify()) {
        return [
            {
                page: YakitRoute.Codec,
