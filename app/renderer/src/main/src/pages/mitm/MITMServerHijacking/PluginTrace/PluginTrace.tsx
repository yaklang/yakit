...
import {useCreation, useDebounceFn, useEventListener, useMemoizedFn, useUpdateEffect, useWhyDidYouUpdate} from "ahooks"
...
export const PluginTrace: React.FC<PluginTraceProps> = React.memo(
    React.forwardRef((props, ref) => {
        const {
            ...
            pluginTraceStats,
            pluginTraceList,
            onSetNoRefreshPluginTraces
        } = props

        useImperativeHandle(
            ref,
            () => ({
                noDetailFun: noDetailFun
            }),
            []
        )
        ...
        useEventListener("mouseenter", () => onSetNoRefreshPluginTraces(true), {target: tableRef.current?.containerRef})
        useEventListener(
            "mouseleave",
            () => {
                onSetNoRefreshPluginTraces(false)
                syncTracesToState()
            },
