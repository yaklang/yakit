...
export const grpcStartPluginTrace: APIFunc<PluginTraceRequest, null> = (params, hiddenError) => {
  return new Promise((resolve, reject) => {
    ...
    ipcRenderer
      ...
      .then(resolve)
      .catch((e) => {
        if (!hiddenError) yakitNotify('error', tOriginal('MITMHacker.grpc_startplugintrace_failed') + e)
        reject(e)
      })
  })
}
...
export const grpcStopPluginTrace: APINoRequestFunc<null> = (hiddenError) => {
  return new Promise((resolve, reject) => {
    ...
    ipcRenderer
      ...
      .then(resolve)
      .catch((e) => {
        if (!hiddenError) yakitNotify('error', tOriginal('MITMHacker.grpc_stopplugintrace_failed') + e)
        reject(e)
      })
  })
}
...
export const grpcPluginTraceIDCancel: APIFunc<string, null> = (traceID, hiddenError) => {
  return new Promise((resolve, reject) => {
    ...
    ipcRenderer
      ...
      .then(resolve)
      .catch((e) => {
        if (!hiddenError) yakitNotify('error', tOriginal('MITMHacker.grpc_plugintraceidcancel_failed') + e)
        reject(e)
      })
  })
}
