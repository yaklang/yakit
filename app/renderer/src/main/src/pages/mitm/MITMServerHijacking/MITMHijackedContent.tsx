...
const MITMHijackedContent: React.FC<MITMHijackedContentProps> = React.memo((props) => {
    ...
    const alertMsg = useMemo(() => {
        ...
        if (openRepRuleFlag && loadedPluginLen) {
            return (
                <Trans
                    i18nKey='MITMHijackedContent.plugin_replace_rule_tip'
                    ns='mitm'
                    components={{
                        close: closeDisposition("rule"),
                        openReplace: openReplaceRule(),
                        tooltip: (
                            <Tooltip title={t("MITMHijackedContent.close_rule_tip2")}>
                                <OutlineInformationcircleIcon className={styles["circle-icon"]} />
                            </Tooltip>
                        ),
                        clear: clearLoadedPlugins()
        ...
        if (whiteListFlag)
            return (
                <Trans
                    i18nKey='MITMHijackedContent.plugin_white_filter_tip_simple'
                    ns='mitm'
                    components={{
                        close: closeDisposition("filter"),
                        openWhite: openWhiteFilter(),
                        tooltip: (
                            <Tooltip title={t("MITMHijackedContent.close_filter_tip2")}>
                                <OutlineInformationcircleIcon className={styles["circle-icon"]} />
                            </Tooltip>
                        )
                    }}
        ...
        if (openRepRuleFlag)
            return (
                <Trans
                    i18nKey='MITMHijackedContent.plugin_replace_rule_tip_simple'
                    ns='mitm'
                    components={{
                        close: closeDisposition("rule"),
                        openReplace: openReplaceRule(),
                        tooltip: (
                            <Tooltip title={t("MITMHijackedContent.close_rule_tip3")}>
                                <OutlineInformationcircleIcon className={styles["circle-icon"]} />
                            </Tooltip>
                        )
                    }}
        ...
        return ""
    }, [openRepRuleFlag, whiteListFlag, loadedPluginLen, i18n.language])
    useEffect(() => {
        if (alertMsg === "") {
