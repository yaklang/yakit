...
function usePluginTrace(params: PluginTraceParams) {
    const {noRefreshPluginTraces, onStart, onError, onEnd} = params
    ...
    const {run: syncTracesToState, cancel: cancelTraces} = useThrottleFn(
        () => {
            if (noRefreshPluginTraces) {
                return
            }
    ...
    const refreshNow = useMemoizedFn(() => {
        setIsRefreshPluginTraces((prev) => !prev)
    })
    ...
    useEffect(() => {
        ...
        ipcRenderer.on("mitm-plugin-trace-update", (event, data) => {
            if (data.Traces.length > 0) {
                console.log('traces')
                let newTraces = cloneDeep(tracesRef.current)
    ...
    return [
        {isInitTrace, startLoading, tracing, stopLoading, isRefreshPluginTraces},
        {
            startPluginTrace,
            stopPluginTrace,
