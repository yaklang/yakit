...
const parseSNI = (raw: string, defaults = createDefaultAdvancedConfig()) => {
  ...
  return {
    OverwriteSNI: typeof sniValue.OverwriteSNI === 'boolean' ? sniValue.OverwriteSNI : sniValue.OverwriteSNI === 'true',
    SNI: sniValue.SNI || '',
    SNIMapping:
      Array.isArray(sniValue.SNIMapping) && sniValue.SNIMapping.length ? sniValue.SNIMapping : defaults.SNIMapping,
  }
}
...
export type { AdvancedConfigurationFromValue }
