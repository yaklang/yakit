...
export const PluginExecutor: React.FC<PluginExecutorProp> = (props) => {
    ...
    return (
        <div>
            <Card
                title={`模块执行操作台：${script.ScriptName}`}
                size={"small"}
                extra={
                    <Space>
                        <Popconfirm
                        ...
                        >
                            <Button size={"small"} disabled={!loading} type={"primary"} danger={true}>
                                立即停止该任务 / KILL TASK
                            </Button>
                        </Popconfirm>
                        <Button
            ...
            >
                <Tabs
                    ...
                    onChange={setTab}
                    activeKey={tabKey}
                    tabBarExtraContent={<Tag>123</Tag>}
                >
                    <Tabs.TabPane key={`params-${randomString(100)}`} tab={"设置参数 / Params"}>
                    ...
                    <Tabs.TabPane key={`console-${randomString(100)}`} tab={"执行结果 / Results"}>
