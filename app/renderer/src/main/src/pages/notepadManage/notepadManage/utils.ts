...
function noteForUI(row: GrpcOutput<'QueryNote'>['Data'][number]) {
  return { ...row, CreateAt: int64ToSafeNumber(row.CreateAt), UpdateAt: int64ToSafeNumber(row.UpdateAt) }
}
...
export const onOpenLocalFileByPath: APIFunc<string, boolean> = (path, hiddenError) => {
  return new Promise((resolve, reject) => {
    ipc
      .invoke('local', 'is-file-exists', path)
      .then((flag: boolean) => {
        if (flag) {
...
export const grpcQueryNote: APIFunc<QueryNoteRequest, QueryNoteResponse> = (params, hiddenError) => {
  return new Promise(async (resolve, reject) => {
    ipc
      .invoke('grpc', 'QueryNote', params)
      .then((res) => grpcPageForUI({ ...res, Data: res.Data.map(noteForUI) }))
      .then(resolve)
      .catch((e) => {
 ...
 * @description 查询笔记本
 */
export const grpcQueryNoteById: APIFunc<string | number, Note> = (id, hiddenError) => {
  return new Promise(async (resolve, reject) => {
...
export const grpcSearchNoteContent: APIFunc<SearchNoteContentRequest, SearchNoteContentTreeResponse> = (
  ...
  return new Promise(async (resolve, reject) => {
    ipc
      .invoke('grpc', 'SearchNoteContent', params)
      .then((res) => {
        const data = SearchNoteContentGroupById(
          res.Data.map((row) => {
            if (!row.Note) throw new Error('笔记搜索结果缺少笔记内容')
            return {
              ...row,
              Note: noteForUI(row.Note),
              Index: int64ToSafeNumber(row.Index),
              Line: int64ToSafeNumber(row.Line),
            }
          }),
        )
 ...
 */
const SearchNoteContentGroupById = (arr: Omit<NoteContent, 'Id'>[]) => {
  const map = {}
  const keys: string[] = []
...
export const showSaveDialog: APIOptionalFunc<string, ShowSaveDialogResponse> = (fileName, hiddenError) => {
  return new Promise((resolve, reject) => {
    ipc
      .invoke('local', 'show-save-dialog', fileName ?? '')
      .then(resolve)
      .catch((err) => {
