          ...
          <div className={styles['user-item']}>
            {ele.head_img ? (
              ...
              <Avatar
                ...
                size={28}
                style={{ backgroundColor: ele.avatarColor || randomAvatarColor(), cursor: 'pointer' }}
              >
  ...
  const collaboratorList: NotepadCollaboratorInfoProps[] = useCreation(() => {
    const newCollaborator: NotepadCollaboratorInfoProps[] =
      collaborators?.map((ele) => ({
        ...ele,
        imgNode: judgeAvatar({ companyHeadImg: ele.head_img, companyName: ele.user_name }, 28, randomAvatarColor()),
      })) || []
            ...
            <div className={styles['collaborator-list']}>
              {collaboratorList?.map((item, index) =>
                item.user_id === notepadInfo.notepadUserId ? (
                  <div key={item.user_id} className={classNames(styles['manage-item'], styles['manage-item-author'])}>
                    <div className={styles['user-item']}>
                      {item.imgNode}
        ...
        <>
          <div className={styles['notepad-user-select']}>
            ...
            <YakitSelect
              ...
              tagRender={(props) => {
                return (
                  <YakitTag className={styles['user-tag-render']} closable={true} onClose={props.onClose}>
                    {props.label}
                  </YakitTag>
                )
              }}
            ...
            />
            <AuthPopover currentRole={selectAuth} onSelect={setSelectAuth} className={styles['user-authority']} />
          </div>
