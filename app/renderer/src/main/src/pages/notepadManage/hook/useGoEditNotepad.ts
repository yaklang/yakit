...
const toEditNotepad = (params: ToEditNotepadProps) => {
  ...
  const current =
    notepadHash && notepadPageList.find((ele) => ele.pageParamsInfo.modifyNotepadPageInfo?.notepadHash === notepadHash)
  if (current) {
...
export const useGoEditNotepad = (props?: GoEditNotepadProps) => {
  ...
  const goEditNotepad = useMemoizedFn((params: ModifyNotepadPageInfoProps) => {
    ...
    const current = notepadPageList.find((ele) => ele.pageParamsInfo.modifyNotepadPageInfo?.notepadHash === notepadHash)
    if (current) {
