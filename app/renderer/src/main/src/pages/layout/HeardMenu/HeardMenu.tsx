...
const HeardMenu: React.FC<HeardMenuProps> = React.memo((props) => {
    ...
    const {t, i18n} = useNamespace(["layout", "yakitRoute", "yakitUi"])
    // 专家模式菜单数据
                                        ...
                                        overlay={
                                            <>
                                                ...
                                                <YakitSpin spinning={loading} tip='Loading...' size='small'>
                                                    ...
                                                    >
                                                        {t(
                                                            CustomizeMenuData.find((ele) => patternMenu === ele.key)
                                                                ?.tipUi as string
                                                        )}
                                                    </div>
                                                    <div
                                        ...
                                        <div className={style["sub-menu-expand"]}>
                                            {(!isDisable && (
                                                ...
                                                >
                                                    ...
                                                    {(loading && nodeLabel) || (
                                                        <Tooltip
                                                            title={t("Layout.HeardMenu.pluginMissingDownload")}
                                                            placement='bottom'
                                                        >
                                                            {nodeLabel}
                                                        </Tooltip>
...
const RouteMenuDataItem: React.FC<RouteMenuDataItemProps> = React.memo((props) => {
    ...
    const {t, i18n} = useNamespace(["yakitRoute"])
    ...
    const popoverContent = (
        ...
        >
            <Tooltip title={menuItem.labelUi ? t(menuItem.labelUi) : menuItem.label} placement='top'>
...
const SubMenu: React.FC<SubMenuProps> = (props) => {
    ...
    const {t, i18n} = useNamespace(["yakitRoute", "layout"])
