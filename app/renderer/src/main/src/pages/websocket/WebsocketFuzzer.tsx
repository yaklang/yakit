          ...
          <div className={styles['websocketClientOperator-card-body']}>
            ...
            <div style={{ display: mode === 'response' ? 'block' : 'none', height: '100%' }}>
              <YakitSpin spinning={!ursp} tip={t('WebsocketFuzzer.buildingConnection')}>
                <YakitEditor type="http" value={ursp} readOnly={true} noMiniMap={true} noLineNumber={true} />
              </YakitSpin>
            </div>
          </div>
        </YakitCard>
              ...
              <YakitPopover
                ...
                content={
                  <div style={{ display: 'flex', justifyContent: 'space-evenly', alignItems: 'center' }}>
                    ...
                    <YakitSwitch checked={clearSendData} onChange={() => setClearSendData((pre) => !pre)} />
                  </div>
                }
              >
...
const WebsocketFlowViewer: React.FC<WebsocketFlowViewerProp> = memo((props) => {
  ...
  const { t, i18n } = useI18nNamespaces(['websocket'])
          ...
          title={
            <div className={styles['websocketClientOperator-card-title']}>
              <span className={styles['websocketClientOperator-card-title-text']}>
                {t('WebsocketFuzzer.realtimePreview')}
              </span>
            </div>
          }
        >
        ...
        currSelected ? (
          <YakitCard
            ...
            title={
              <div className={styles['websocketClientOperator-card-title']}>
                <span className={styles['websocketClientOperator-card-title-text']}>
                  {t('WebsocketFuzzer.frameDetails')}
                </span>
                ...
                <YakitTag color="info">
                  {t('WebsocketFuzzer.dataSize', { size: currSelected.DataSizeVerbose })}
                </YakitTag>
              </div>
            }
            extra={
          ...
          <YakitEmpty title={t('WebsocketFuzzer.selectDataFrame')} className={styles['webSocket-fuzzer-empty']} />
        )
      }
      secondRatio="50%"
      secondMinSize="400px"
