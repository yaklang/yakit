...
export const ScreenRecorderPage: React.FC<ScreenRecorderPageProp> = (props) => {
    ...
    const init = () => {
        setLoading(true)
        ipcRenderer.invoke("IsScrecorderReady", {}).then((data: { Ok: boolean, Reason: string }) => {
    ...
    return <div>
        <PageHeader title={<Space>
            录屏管理器
            <YakitButton loading={loading} type={"outline1"} icon={<RefreshIcon/>} onClick={init}>刷新状态</YakitButton>
        </Space>} extra={<div>
            <Space>
                <Popconfirm title={"安装录屏依赖需要联网"}
                            onConfirm={() => {
                                const m = showDrawer({
                                    title: "安装录屏相应组件操作",
                                    content: (
        ...
        <AutoSpin spinning={loading}>
            <ResizeBox
                firstNode={<AutoCard size={"small"} bordered={false} title={"录屏基础设置"}>
                    <ScreenRecorderForm onRefresh={() => {
...
export const ScreenRecorderForm: React.FC<ScreenRecorderFormProp> = (props) => {
    ...
    return <>
        <Alert type={"success"} style={{marginBottom: 24}} message={<div>
            <Space>
                <Typography.Paragraph>
                    本录屏在 Windows 下会 <Typography.Text mark={true}>
                    同时录制所有屏幕
                </Typography.Text> 合并在一个文件中
                </Typography.Paragraph>
            </Space>
            <br/>
            <Typography.Paragraph>
        ...
        <Form
            size={"small"} labelCol={{span: 5}} wrapperCol={{span: 14}}
            onSubmitCapture={e => {
                e.preventDefault()
        ...
        >
            ...
            <ManySelectOne
                ...
                setValue={CoefficientPTS => setParams({...params, CoefficientPTS})} value={params.CoefficientPTS}
            />
            <SwitchItem
                label={"禁止鼠标捕捉"} setValue={DisableMouse => setParams({...params, DisableMouse})}
                value={params.DisableMouse}
                disabled={recording}
            />
            <Form.Item colon={false} label={" "}>
                <Space>
                    <YakitButton loading={recording} type="primary" htmlType="submit"> 开始录屏 </YakitButton>
                    <Popconfirm title={"确定要停止录屏吗？"} onConfirm={() => {
                        ipcRenderer.invoke("cancel-StartScrecorder", token)
                        if (props?.onRefresh) {
                            props.onRefresh()
                        }
