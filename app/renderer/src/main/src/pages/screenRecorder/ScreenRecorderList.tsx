...
export const ScreenRecorderList: React.FC<ScreenRecorderListProp> = (props) => {
    ...
    const update = useMemoizedFn((page?: number, limit?: number, order?: string, orderBy?: string, extraParam?: any, reload?: boolean) => {
        ...
        ipcRenderer.invoke("QueryScreenRecorders", {
    ...
    return <AutoCard title={"当前项目录屏列表"} size={"small"} bordered={true} extra={<div>
        <YakitButton type={"outline2"} onClick={() => update(1)}>刷新</YakitButton>
    </div>}>
        <List<ScreenRecorder>
            dataSource={data}
            pagination={false}
            ...
            renderItem={item => {
                return <List.Item key={item.Id}>
                    ...
                    >
                        <YakitButton onClick={e => {
                            showByCursorMenu({
                                content: [
                                    {
                                        title: "打开文件位置", onClick: () => {
                        ...
                        }} type={"outline2"}>文件位置：{item.Filename}</YakitButton>
                    </AutoCard>
                </List.Item>
            }}
        />
