...
import {AIFileSystemPin, UseAIPerfDataState, UseCasualChatState, UseChatIPCEvents, UseTaskChatState, UseYakExecResultState} from "./type"
...
function useAIChatUIData() {
    ...
    const runTimeIDs: string[] = useCreation(() => {
        if (activeChat && activeChat.session && aiChatDataStore) {
            return aiChatDataStore.get(activeChat.session)?.runTimeIDs || chatIPCData.runTimeIDs
        }
    ...
    const grpcFolders: AIFileSystemPin[] = useCreation(() => {
        if (activeChat && activeChat.session && aiChatDataStore) {
            return aiChatDataStore.get(activeChat.session)?.grpcFolders || chatIPCData.grpcFolders
        }
    ...
    const taskChat: UseTaskChatState = useCreation(() => {
        if (activeChat && activeChat.session && aiChatDataStore) {
            return aiChatDataStore.get(activeChat.session)?.taskChat || chatIPCData.taskChat
        }
    ...
    const yakExecResult: UseYakExecResultState = useCreation(() => {
        if (activeChat && activeChat.session && aiChatDataStore) {
            return aiChatDataStore.get(activeChat.session)?.yakExecResult || chatIPCData.yakExecResult
        }
    ...
    const aiPerfData: UseAIPerfDataState = useCreation(() => {
        if (activeChat && activeChat.session && aiChatDataStore) {
            return aiChatDataStore.get(activeChat.session)?.aiPerfData || chatIPCData.aiPerfData
        }
    ...
    const casualChat: UseCasualChatState = useCreation(() => {
        if (activeChat && activeChat.session && aiChatDataStore) {
            return aiChatDataStore.get(activeChat.session)?.casualChat || chatIPCData.casualChat
        }
    ...
    const reActTimelines = useCreation(() => {
        if (activeChat && activeChat.session && aiChatDataStore) {
            return aiChatDataStore.get(activeChat.session)?.reActTimelines || chatIPCData.reActTimelines
        }
    ...
    const coordinatorIDs: string[] = useCreation(() => {
        if (activeChat && activeChat.session && aiChatDataStore) {
            return aiChatDataStore.get(activeChat.session)?.coordinatorIDs || chatIPCData.coordinatorIDs
        }
