...
interface SSADiffResponse {
    ...
    CompareRisk: SSARisk
    RuleName: string
    Status: number // 0 - 绿色 相等（不用显示） 1 - 右侧增加 2 - 右侧减少 左侧显示红色
}
...
const SsaResDiff: React.FC<SsaResDiffProps> = React.memo((props) => {
    ...
    const [baseProgram, setBaseProgram] = useState<string>("")
    const [compareProgram, setCompareProgram] = useState<string>("")
        ...
        <div className={styles["ssaResDiff"]}>
            <div className={styles["ssaResDiff-header"]}>
                <div className={styles["ssaResDiff-ssaDiffItem"]}>
                    <span>选择扫描结果</span>
                    <YakitSelect
                        placeholder='请选择项目后对比'
                        options={project}
                        notFoundContent='暂无数据'
                        onChange={(value) => setBaseProgram(value)}
                    />
                </div>
                <div className={styles["ssaResDiff-ssaDiffItem"]}>
                    <span>对比扫描结果</span>
                    <YakitSelect
                        ...
                        options={project}
                        notFoundContent='暂无数据'
                        onChange={(value) => setCompareProgram(value)}
                    />
                </div>
                <YakitButton type='primary' onClick={onStartDiff} disabled={!(baseProgram && compareProgram)}>
                    开始对比
                </YakitButton>
            </div>
            <div className={styles["ssaResDiff-resule-wrapper"]}>
                <YakitResizeBox
                    freeze={true}
                    firstNode={
                        <div className={styles["ssaResTable-BaseRisk"]}>
                            <SsaResTableAndDetail
                                tableData={(ssaDiffRes?.map((item) => item.BaseRisk) || []).filter(
                                    (item) => item.Id != 0
                                )}
                                isRefreshTable={isRefreshBaseTable}
                            />
                        </div>
                    }
                    firstMinSize={300}
                    secondNode={
                        <div className={styles["ssaResTable-CompareRisk"]}>
                            <SsaResTableAndDetail
                                tableData={(ssaDiffRes?.map((item) => item.CompareRisk) || []).filter(
                                    (item) => item.Id != 0
                                )}
                                isRefreshTable={isRefreshCompareTable}
                            />
                        </div>
                    }
                    secondMinSize={300}
