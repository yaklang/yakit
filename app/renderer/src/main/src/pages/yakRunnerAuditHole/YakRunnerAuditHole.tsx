...
import type { VulnerabilityLevelPieRefProps } from '../risks/VulnerabilityLevelPie/VulnerabilityLevelPieType'
import { VulnerabilityLevelPie } from '../risks/VulnerabilityLevelPie/VulnerabilityLevelPie'
import type { VulnerabilityTypePieRefProps } from '../risks/VulnerabilityTypePie/VulnerabilityTypePieType'
...
import { type PageNodeItemProps, type AuditHoleInfoProps, usePageInfo } from '@/store/pageInfo'
import { YakitRoute } from '@/enums/yakitRoute'
import { defaultAuditHolePageInfo } from '@/defaultConstants/defaultAuditHolePageInfo'
import type { LeftSideHoleType } from './LeftSideHoleBar/LeftSideHoleBarType'
...
export const YakRunnerAuditHole: React.FC<YakRunnerAuditHoleProps> = (props) => {
  ...
  useEffect(() => {
    const auditHoleVulnerabilityLevel = (params: string) => {
      try {
        const data: AuditHoleInfoProps = JSONParseLog(params, {
          page: 'YakRunnerAuditHole',
          fun: 'auditHoleVulnerabilityLevel',
        })
        ...
        <YakitResizeBox
          ...
          firstNode={
            <LeftSideHoleBar
              ...
              statisticNode={
                <HoleQuery inViewport={inViewport} onOperateSide={onOperateSide} query={query} setQuery={setQuery} />
              }
          ...
          secondNode={<YakitAuditHoleTable setRiskLoading={setRiskLoading} query={query} setQuery={setQuery} />}
        />
      </div>
    </YakitSpin>
  )
      ...
      <div className={styles['program-list-content']}>
        <RollingLoadList<FieldGroup>
          ...
          renderRow={(record, index: number) => (
            <ProgramListItem item={record} onSelect={onSelect} isSelect={selectList.includes(record.Name)} />
          )}
        />
      </div>
    </div>
  ...
  return (
    ...
    >
      <Tooltip title={item.Name}>
        <div className={classNames(styles['program-list-item-label'], 'yakit-single-line-ellipsis')}>{item.Name}</div>
      </Tooltip>
