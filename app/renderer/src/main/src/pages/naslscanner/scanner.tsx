...
import {
    ...
    YakPoCProps
} from "../securityTool/yakPoC/YakPoCType"
import classNames from "classnames"
import styles from "../securityTool/yakPoC/YakPoC.module.scss"
...
import {apiFetchQueryYakScriptGroupLocalByPoc} from "../securityTool/yakPoC/utils"
...
export interface NASLScannerProps {}
/**专项漏洞检测 */
export const NASLScanner: React.FC<NASLScannerProps> = React.memo((props) => {
    const pageId = "1"
...
const PluginListByGroup: React.FC<PluginListByGroupProps> = React.memo((props) => {
    ...
    const fetchList = useDebounceFn(
        useMemoizedFn(async (reset?: boolean) => {
            ...
            try {
                const res = await ipcRenderer.invoke("QueryNaslScript", query)
                console.log("res", res)
...
const PluginGroupGrid: React.FC<PluginGroupGridProps> = React.memo((props) => {
    ...
    const getQueryYakScriptGroup = useMemoizedFn(() => {
        setLoading(true)
        ipcRenderer
            .invoke("GetNaslFamilies", {Name: ""})
            .then((res: {Families: {Name: string; ScriptCount: number}[]}) => {
                let groupCount: GroupCount[] = []
                for (let i = 0; i < res.Families.length; i++) {
                    groupCount.push({
                        Value: res.Families[i]?.Name || "",
                        Total: res.Families[i]?.ScriptCount || 0,
                        Default: false
                    })
                }
                initialResponseRef.current = groupCount
                const newSelectGroupList = selectGroupList.filter((item) =>
                    groupCount.some((ele) => ele.Value === item)
                )
                setSelectGroupList(newSelectGroupList)
                if (response.length === 0) {
                    setResponse(groupCount)
                    setIsRef(!isRef)
                }
            })
            .catch((e) => {
