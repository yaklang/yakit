...
export const PayloadManagerPage: React.FC<PayloadManagerPageProp> = (props) => {
    ...
    const updateDict = (page?: number, limit?: number) => {
        ipcRenderer.invoke("QueryPayload", {
            ...params, Group: selected,
            Pagination: {
                ...params.Pagination, Page: page || params.Pagination.Page,
    ...
    return <div>
        <PageHeader title={"Payload / 字典管理"} subTitle={`增加 / 删除 / 管理字典，可以通过 fuzz 模块 {{x(字典名)}} 来渲染`}/>
        <Row gutter={18}>
            <Col span={8}>
                                  ...
                                  <Button.Group>
                                      <Button size={"small"} onClick={() => {
                                          let m = showModal({
                                              ...
                                              content: <>
                                                  <CreatePayloadGroup Group={""} onFinished={e => {
                ...
                >
                    <List<string>
                        ...
                        renderItem={(element, index) => {
                            return <List.Item id={index.toString()}>
                                ...
                                >
                                    ...
                                    >字典分组名：{element}</Button>
                                    <Popconfirm
                                        title={"确定删除该字典吗？"}
                                        onConfirm={e => {
                                            ipcRenderer.invoke("DeletePayloadByGroup", {
                                                Group: element,
                                            }).then(() => {
                                                updateGroup()
                                            }).catch(e => {
                                    ...
                                    >
                                        <Button
                                            danger={true} type={selected === element ? "primary" : undefined}
                                        >删除字典</Button>
                                    </Popconfirm>
                                </Button.Group>
                            </List.Item>
                        ...
                        {title: "字典内容", render: (e: Payload) => e.Content},
                            ...
                            total: response?.Total || 0, showTotal: (i) => <Tag>共{i}条历史记录</Tag>,
                        }}
                    ...
                    <InputItem label={"字典名/组"} setValue={Group => setParams({...params, Group})} value={params.Group}
                    required={true}
                    />
                    ...
                    <Button type="primary" htmlType="submit"> 创建新的字典 </Button>
                    </Form.Item>
                    </Form>
                    </div>
                };