  ...
  return (
    <div style={{ marginTop: 20 }}>
      ...
      >
        ...
        <InputInteger
          label={'反连服务器端口'}
          setValue={(LocalFacadePort) => setParams({ ...params, LocalFacadePort })}
          value={params.LocalFacadePort}
        />
        <SwitchItem
          label={'DNSLog'}
          setValue={(EnableDNSLogServer) => setParams({ ...params, EnableDNSLogServer })}
          value={params.EnableDNSLogServer}
          help={'在本地启动一个 DNS 服务器(UDP)'}
        />
        {params.EnableDNSLogServer && (
          <>
            <InputInteger
              label={'DNSLog 本地端口'}
              setValue={(DNSLogLocalPort) => setParams({ ...params, DNSLogLocalPort })}
              value={params.DNSLogLocalPort}
            />
          </>
        )}
        {props.remoteMode && (
