...
const ConcurrencyAllRes = React.lazy(() => import("./ConcurrencyAllRes").then(({ConcurrencyAllRes}) => ({default: ConcurrencyAllRes})))
...
const FuzzerSequence: React.FC<FuzzerSequenceProps> = React.memo((props) => {
    ...
    // 序列模式的响应数据
    const [sequenceResponseMap, {set: setSequenceResponse, get: getSequenceResponse, reset: resetSequenceResponse}] = useMap<string, ResponseProps>()
    // 并发模式的响应数据
    ...
    const { startConcurrency, cancelConcurrency, loading: concurrencyLoading } = useStreamConcurrency<FuzzerSequenceResponse>({
        onData: (data) => {
    ...
    //根据tab展示对应的loading状态
    const loading = useMemo(() => isConcurrency ? concurrencyLoading : sequenceLoading, [isConcurrency, concurrencyLoading, sequenceLoading])
    ...
    useEffect(() => {
        if (!currentSequenceItem?.pageId) return
        const unsubscribe = usePageInfo.subscribe(
            (state) => state.queryPagesDataById(YakitRoute.HTTPFuzzer, currentSequenceItem.pageId),
            (newData) => {
                const webFuzzerPageInfo = newData?.pageParamsInfo?.webFuzzerPageInfo
                if (!webFuzzerPageInfo) return
                setCurrentSelectRequest((prev) => {
                    if (prev && isEqual(prev, webFuzzerPageInfo)) {
                        return prev
                    }
                    return {...webFuzzerPageInfo}
                })
                setHotPatchCodeRef(webFuzzerPageInfo.hotPatchCode)
            }
        )
        return () => {
            unsubscribe()
        }
    }, [currentSequenceItem?.pageId])
    useThrottleEffect(
        () => {
    ...
    useEffect(() => {
        ...
        ipcRenderer.on(dataToken, (e: any, data: FuzzerSequenceResponse) => {
            ...
            if (rematchTargetRef.current && rematchTargetRef.current !== FuzzerIndex) {
                return
            }
    ...
    const onConcurrencyMatchSubmit = useMemoizedFn(() => {
        ...
        const { matchers= [], extractors=[], repeatTimes, disableUseConnPool, concurrent, minDelaySeconds, maxDelaySeconds } = ConcurrencyAdvancedConfigValue
    ...
    const onSingleMatchSubmit = useMemoizedFn(() => {
        ...
        try {
            ...
            const requestIndex = allHttpParams.findIndex((p) => p.FuzzerIndex === currentSequenceItem.id)
            if (requestIndex === -1) return

            const cloneMatchers = () => matchers.map((item) => ({...item}))
            const cloneExtractors = () => extractors.map((item) => ({...item}))

            let requestPayload: FuzzerRequestProps[] = []
            if (isConcurrency) {
                const currentHttpParam = allHttpParams[requestIndex]
                requestPayload = [
                    {
                        ...currentHttpParam,
                        Matchers: cloneMatchers(),
                        Extractors: cloneExtractors()
                    }
                ]
            } else {
                rematchTargetRef.current = currentSequenceItem.id
                requestPayload = allHttpParams.map((item, index) => {
            ...
            if (isConcurrency) {
                startConcurrency({Requests: requestPayload})
            } else {
                ipcRenderer.invoke("HTTPFuzzerSequence", {Requests: requestPayload}, fuzzTokenRef.current)
            }
            setHasExtractorRules(true);
        } catch (error) {
            if (!isConcurrency) {
    ...
    const onSaveMatcherAndExtractionDrawer = useMemoizedFn(
        (matcher: MatcherValueProps, extractor: ExtractorValueProps, isApply?: boolean ) => {
            ...
            if (
                ...
                updatePagesDataCacheById(YakitRoute.HTTPFuzzer, {...newCurrentItem})
                setCurrentSelectRequest((prev) => {
                    if (!prev || prev.pageId !== currentItem.pageId) return prev
                    return {
                        ...prev,
                        advancedConfigValue: {
                            ...prev.advancedConfigValue,
                            matchers: matcher.matchersList || [],
                            extractors: extractor.extractorList || []
                        }
                    }
    ...
    const renderHistoryAnalysis = useMemoizedFn(() => {
        ...
        return (
            <YakitDrawer
                title={isSingle ? `${t('HTTPFuzzerPage.trafficAnalysisMode')}-${currentItem?.pageName}` : t('SequenceResponseHeard.show_all_responses')}
