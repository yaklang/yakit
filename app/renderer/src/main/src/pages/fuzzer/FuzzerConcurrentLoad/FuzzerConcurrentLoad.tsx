...
import { type ConcurrentLoad, concurrentLoad } from '@/utils/duplex/duplex'
      ...
      echartsInstance.setOption(
        {
          ...
          title: {
            subtext:
              (type === 'rps'
                ? t('RpsAndCpsLineChart.currentPacketCount')
                : t('RpsAndCpsLineChart.currentConnectionCount')) + (dataArr[dataArr.length - 1]?.number || 0),
          },
        },
        false,
        true,
...
const RequestDelayStackedAreaChart: React.FC<RequestDelayStackedAreaChartProps> = ({
  ...
  const optionRef = useRef<echarts.EChartsOption>({
    ...
    legend: {
      ...
      left: '45%',
      data: [
        t('RequestDelayStackedAreaChart.tlsHandshake'),
        t('RequestDelayStackedAreaChart.tcpConnection'),
        t('RequestDelayStackedAreaChart.totalDuration'),
      ],
      textStyle: {
...
export const FuzzerConcurrentLoad: React.FC<FuzzerConcurrentLoadProps> = React.memo((props) => {
  ...
  return (
    <>
      ...
      />
      <RequestDelayStackedAreaChart fuzzerResChartData={fuzzerResChartData} inViewportCurrent={inViewportCurrent} />
      <DurationMsLineChart
        fuzzerResChartData={fuzzerResChartData}
        inViewportCurrent={inViewportCurrent}
        lineChartOption={{
