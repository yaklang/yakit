...
export const YakScriptParamsSetter: React.FC<YakScriptParamsSetterProps> = (props) => {
    ...
    const yakScriptParamToNode = (i: YakScriptParam, required: boolean, keyValue: string) => {
        return (
            <Form.Item
                key={keyValue}
                ...
                >
                    ...
                    {requiredParams.length > 0 && (
                        <>
                            ...
                            {requiredParams.map((i, index) => yakScriptParamToNode(i, true, `requiredParams-${index}`))}
                        </>
                    )}
                    {extraGroup.length <= 1 ? (
                        <>
                            ...
                            {!isGroupHidden("default", false) && (
                                <>
                                    {(groupToParams.get("default") || []).map((i: YakScriptParam, index) =>
                                        yakScriptParamToNode(i, false, `defaultParamsGroup-${index}`)
                                    )}
                                </>
                            )}
                        </>
                                ...
                                return (
                                    <>
                                        ...
                                        {!isGroupHidden(i, true) && (
                                            <>
                                                {(groupToParams.get(i) || []).map((i: YakScriptParam, index) =>
                                                    yakScriptParamToNode(i, false, `ParamsGroup-${index}`)
                                                )}
                                            </>
                                        )}
                                    </>
                    ...
                    <Form.Item colon={false} label={" "}>
                        <Button type="primary" htmlType="submit">
                            {" "}
                            {props.submitVerbose ? props.submitVerbose : "提交已设置的参数"}{" "}
                        </Button>
                    </Form.Item>
                </Form>
            </Card>
