...
export const YakitLogFormatter: React.FC<YakitLogFormatterProp> = React.memo((props) => {
  ...
  const renderContent = useMemoizedFn(() => {
    switch (level) {
      case 'file':
        try {
          const obj = JSONParseLog(data, { page: 'YakitLogFormatter', fun: 'file' }) as
            | PluginExecuteLogFile.FileItem
            | FileLogShowDataProps

          if (isPluginExecuteLogFileItem(obj)) {
            return (
              <ExecuteLogFile {...(obj as PluginExecuteLogFile.FileItem)} timestamp={timestamp} showTime={showTime} />
            )
          } else {
            return <FileLogShow {...(obj as FileLogShowDataProps)} timestamp={timestamp} showTime={showTime} />
          }
        } catch (e) {
          return (
      ...
      case 'json-httpflow-risk':
        try {
          return (
            <HTTPFlowRiskViewer
              risk={JSONParseLog(data, { page: 'YakitLogFormatter', fun: 'json-httpflow-risk' }) as YakitHTTPFlowRisk}
            />
          )
        } catch (e) {
          return <span className={styles['log-time']}>{formatTime(timestamp)}</span>
...
const ExecuteLogFile: React.FC<ExecuteLogFileProps> = React.memo((props) => {
  const { path, action, action_message, showTime, timestamp } = props
  const [expand, setExpand] = useState<boolean>(true)
  const [fileName, setFileName] = useState<string>('')
  useEffect(() => {
    getLocalFileName(path).then((res) => {
  ...
  return (
    <div className={styles['log-file']}>
      <div className={styles['log-file-heard']}>
        ...
        {actionsStatus.message && <YakitTag color={actionsStatus.color}>{actionsStatus.message}</YakitTag>}
      </div>
      ...
      >
        <div className={styles['log-file-body']}>{actionsStatus.content}</div>
      </YakitCard>
    </div>
  )
})
