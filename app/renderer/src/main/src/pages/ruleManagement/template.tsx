...
export const LocalRuleGroupList: React.FC<LocalRuleGroupListProps> = memo((props) => {
    ...
    const handleSelect = useMemoizedFn((info: SyntaxFlowGroup) => {
        ...
        if (isExist) setSelect((arr) => arr.filter((item) => item !== info))
        else setSelect((arr) => [...arr, info])
        onGroupChange(getSelect().map((item) => item.GroupName))
    })

    // 更新数据
    const handleUpdateData = useMemoizedFn((type: "modify" | "delete", info: SyntaxFlowGroup) => {
        if (type === "modify") {
            setData((arr) => {
                return arr.map((ele) => {
                    if (ele === info) return info
                    return ele
                })
            })
        }
        if (type === "delete") {
            setData((arr) => {
                return arr.filter((ele) => ele !== info)
            })
        }
    })
    ...
    const handleAddGroupBlur = useMemoizedFn(() => {
        ...
        grpcCreateLocalRuleGroup({GroupName: groupName})
            .then(() => {
                setData((arr) => arr.concat({GroupName: groupName, Count: 0}))
                setTimeout(() => {
                    setActiveAdd(false)
                    setGroupName("")
                }, 200)
    ...
    /** ---------- 编辑 ---------- */
    const [loadingEditKeys, setLoadingEditKeys] = useState<string[]>([])
    ...
    const handleEdit = useMemoizedFn((info: SyntaxFlowGroup) => {
        ...
        const isExits = loadingEditKeys.includes(GroupName)
        if (isExits) return
    ...
    const handleDoubleEditBlur = useMemoizedFn(() => {
        ...
        setLoadingEditKeys((arr) => [...arr, editInfo.GroupName])
        grpcUpdateLocalRuleGroup({Env: 1})
            .then(() => {
                handleUpdateData("modify", {...editInfo, GroupName: editName})
                initEdit()
            })
            .catch(() => {})
            .finally(() => {
                setTimeout(() => {
                    setLoadingEditKeys((arr) => arr.filter((ele) => ele !== editInfo.GroupName))
                }, 200)
            })
    })
    /** ---------- 上传 ---------- */
    ...
    //     const isExits = loadingUploadKeys.includes(`${info}`)
    //     if (isExits) return
    ...
    const handleDelete = useMemoizedFn((info: SyntaxFlowGroup) => {
        const {GroupName} = info
        const isExits = loadingDelKeys.includes(GroupName)
        if (isExits) return
                    ...
                    <div ref={wrapperRef} className={styles["list-body"]}>
                        <div ref={bodyRef}>
                            {list.map((item) => {
                                ...
                                const isEditLoading = loadingEditKeys.includes(name)
                        ...
                        {activeAdd && (
                            <div className={styles["list-add-input"]}>
                                <YakitInput
                                    ref={addInputRef}
                                    showCount
                                    maxLength={50}
...
export const EditRuleDrawer: React.FC<EditRuleDrawerProps> = memo((props) => {
    ...
    const handleFormSubmit = useMemoizedFn(() => {
        return new Promise<any | undefined>((resolve) => {
            form.validateFields()
                .then(() => {
                    console.log("handleFormSubmit", handleGetFormData())
                    resolve(handleGetFormData())
                })
                .catch(() => {
                    resolve(undefined)
    ...
    // 更新表单数据
    const handleUpdateFormData = useMemoizedFn((value: Record<string, any>) => {
        if (form) {
        ...
        <YakitDrawer
            ...
            extra={
                <div className={styles["drawer-extra"]}>
                    <YakitButton onClick={handleFormSubmit}>保存</YakitButton>
