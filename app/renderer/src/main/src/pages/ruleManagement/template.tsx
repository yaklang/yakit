...
import { Descriptions, Form, type InputRef, Progress, Tooltip } from 'antd'
...
import { failed, yakitNotify, getModalApi } from '@/utils/notification'
    ...
    return (
      <div className={styles['rule-group-list']}>
        ...
        <div className={styles['list-search-and-add']}>
          <YakitInput.Search
            size="large"
            placeholder={t('LocalRuleGroupList.searchPlaceholder')}
      ...
      return (
        <div className={styles['drawer-title']}>
          ...
          <div className={styles['title-style']}>
            {`${isEdit ? t('YakitButton.edit') : t('YakitButton.new')}${t('EditRuleDrawer.rule')}`} /{' '}
            <span className={styles['active-title']}>{t('EditRuleDrawer.auditDetail')}</span>
          </div>
        </div>
      )
        ...
        <div className={classNames(styles['drawer-body'], { [styles['drawer-hidden']]: auditDetailShow })}>
          ...
          >
            ...
            <div className={styles['info-setting']}>
              <Form layout="vertical" form={form} className={styles['info-setting-form']}>
                ...
                >
                  <YakitSelect allowClear size="large" disabled={isBuildInRule} options={RuleLanguageList} />
                </Form.Item>
          ...
          <div className={styles['rule-code']}>
            <div className={styles['code-header']}>
              ...
              <div className={styles['header-extra']}>
                {!!progress && (
                  <PluginExecuteProgress percent={progress} name={t('EditRuleDrawer.executionProgress')} />
                )}
                      ...
                      >
                        ...
                        >
                          <YakitSelect
                            ...
                            filterOption={false}
                            notFoundContent={t('YakitEmpty.noData')}
      ...
      <div className={styles['audit-detail']}>
        {isShowAuditDetail ? (
          ...
          <div className={styles['no-audit']}>
            <YakitEmpty title={t('YakitEmpty.noData')} description={t('RuleDebugAuditDetail.selectLeft')} />
          </div>
        )}
      </div>
    </div>
      ...
      <div ref={wrapperRef} className={styles['audit-list-wrapper']}>
        <div ref={bodyRef}>
          {list.map((item) => {
            ...
            return (
              <div key={ResultID} className={classNames(styles['audit-opt'])} onClick={() => onDetail(item.data)}>
                <YakitRoundCornerTag>{ResultID}</YakitRoundCornerTag>
                <div className={classNames(styles['opt-title'], 'yakit-content-single-ellipsis')} title={Title || '-'}>
                  {Title || '-'}
                </div>
    ...
    <YakitModal
      ...
      footer={
        <YakitButton type={'outline2'} onClick={onStreamCancel}>
          {stream[0]?.Progress === 1 ? t('YakitButton.finish') : t('YakitButton.cancel')}
        </YakitButton>
      }
    >
                            ...
                            {SeverityMapTag.map((item) => (
                              <div
                                ...
                                className={classNames(styles['severity-menu-item'], {
                                  [styles['active']]: item.key.includes(alert.Severity || ''),
                                })}
                                onClick={() => {
                                  const severity = ['critical', 'high', 'middle', 'low', 'info'].filter((sev) =>
