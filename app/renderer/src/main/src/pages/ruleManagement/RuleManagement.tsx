...
const DefaultPaging = {Page: 1, Limit: 10, OrderBy: "updated_at", Order: "desc"}
...
export const RuleManagement: React.FC<RuleManagementProps> = memo((props) => {
    ...
    const updateList = useMemoizedFn((page?: number) => {
        console.log("updateList", page)
        if (loading) return

        const request: SyntaxFlowRuleFilter = cloneDeep(filters)

        setLoading(true)
        grpcFetchLocalRuleList({Pagination: {...DefaultPaging, Page: page || 1}, Filter: request})
            .then((res) => {
                const {Pagination, Rule, Total} = res
                console.log("updateList-response", res)
                const rules = (data.Rule || []).concat(Rule)
                setData({Pagination, Rule: rules, Total})
            })
            .catch(() => {})
            .finally(() => {
                setTimeout(() => {
    ...
    const fetchTotal = useMemoizedFn(() => {
    ...
    const fetchList = useDebounceFn(
        () => {
            ...
            const pagination = {...DefaultPaging}
            setLoading(true)
            grpcFetchLocalRuleList({Pagination: pagination, Filter: cloneDeep(filters)})
                .then((res) => {
                    const {Pagination, Rule, Total} = res
    ...
    const onRefreshList = useMemoizedFn(() => {
        fetchTotal()
        fetchList()
    })
                        ...
                        renderTitle={
                            <div className={styles["table-header"]}>
                                <div className={styles["header-title"]}>
                                    ...
                                    <TableTotalAndSelectNumber total={allTotal} selectNum={selectNum} />
                                </div>
