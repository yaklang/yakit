...
const FileTreeSystemItem: FC<FileTreeSystemItemProps> = ({
  ...
  const menuData = useMemo(() => {
    const menu = [
      ...
      {
        key: 'path',
        label: t('YakitButton.copyPath'),
        isHide: false,
      },
      {
        key: 'openFolder',
        label: t('YakitButton.showInFolder'),
        isHide: false,
      },
      {
        key: 'closeFolder',
        label: <span style={{ color: 'var(--Colors-Use-Error-Primary)' }}>{t('YakitButton.remove')}</span>,
        isHide: !(data.depth === 1 && isOpen),
      },
    ]
