...
export const ChaosMakerOperators: React.FC<ChaosMakerOperatorsProp> = (props) => {
  ...
  useEffect(() => {
    setParams({
      ...params,
      ExtraOverrideDestinationAddress: availableAddrs.map((i) => {
        return i.Addr
      }),
    })
  }, [availableAddrs])
      ...
      <Form.Item help={'如果添加探针，模拟攻击流量将会额外对探针进行发送'}>
        <div style={{ display: 'flex', gap: '8px' }}>
                              ...
                              <AddBASAgent
                                onFinished={(data) => {
                                  if (data.IsAvailable) {
                                    ...
                                    showYakitModal({
                                      title: '错误提示',
                                      content: <div style={{ margin: 24 }}>添加 Agent 失败：{data.Reason}</div>,
          ...
          {props.couldBeenReset ? (
            ...
            >
              <div style={{ fontWeight: 'bold', color: '#fff' }}>重置 BAS 操作台</div>
            </AutoCard>
          ) : (
            <AutoCard
              style={{
            ...
            >
              <div style={{ fontWeight: 'bold', color: '#fff' }}>配置模拟攻击参数</div>
            </AutoCard>
          )}
        </div>
      </Form.Item>
  ...
  return (
    ...
    >
      <InputItem
        label={'需要导入的地址'}
        required={true}
        setValue={(Addr) => setParams({ ...params, Addr })}
        value={params.Addr}
        disable={loading}
        autoComplete={['127.0.0.1:8787']}
      />
      <InputInteger
        label={'连接超时'}
