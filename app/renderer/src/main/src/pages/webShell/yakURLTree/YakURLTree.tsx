  ...
  return (
    <AutoCard
      title={
        ...
        >
          <InputItem label={'YakURL'} value={yakurl} setValue={setYakURL} />
        </Form>
      }
      size={'small'}
      bodyStyle={{ overflowY: 'hidden', padding: 0, margin: 0 }}
    >
      <YakitResizeBox
        isVer={false}
        firstMinSize={'300ox'}
        ...
        secondNode={<div>{selected ? <div>SELECTED {`${JSON.stringify(selected)}`}</div> : <div>未选择</div>}</div>}
      />
    </AutoCard>
  )
}
