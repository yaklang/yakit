...
import { type PaginationSchema, type YakScript } from '@/pages/invoker/schema'
import { WebShellTable } from '@/pages/webShell/WebShellTable'
import { YakitSwitch } from '@/components/yakitUI/YakitSwitch/YakitSwitch'
import { Divider, Form } from 'antd'
import YakitCollapse from '@/components/yakitUI/YakitCollapse/YakitCollapse'
import { YakitButton } from '@/components/yakitUI/YakitButton/YakitButton'
import { SMViewGridAddIcon } from '@yakit-libs/yakit-ui-icons/oldicon'
  ...
  return loading ? (
    ...
    <div className={styles['cve-viewer']}>
      {available && advancedQuery && (
        <WebShellQuery onChange={setParams} advancedQuery={advancedQuery} setAdvancedQuery={setAdvancedQuery} />
      )}
      <WebShellTable
        // filter={params}
      ...
      <YakitHint
        ...
        content={
          <>
            <div>
              本工具是一款开源的跨平台网站管理工具，旨在为合法授权的安全研究人员、渗透测试专家以及网站管理员提供支持。它的设计目的是为了进行安全评估、教育目的以及帮助网站管理员进行日常管理工作。
            </div>
            <div>
              请用户在使用本工具时遵守所有适用的法律和道德准则。本工具不应用于任何未经授权的测试、非法入侵、数据盗窃或任何其他非法活动。用户使用本工具进行的任何行为都应当是合法、合规且具有正当授权的。
            </div>
          </>
        }
        onOk={() => {
    ...
    <>
      <div className={fuzzerStyles['http-query-advanced-config']}>
        <div className={fuzzerStyles['advanced-config-heard']}>
          <span>高级设置</span>
          <YakitSwitch
            wrapperClassName={fuzzerStyles['btn-padding-right-0']}
            checked={advancedQuery}
