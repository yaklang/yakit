...
const BaseQAContent = forwardRef<{scrollToBottom: () => void}, BaseQAContentProps>(({content}, ref) => {
    // icon的唯一id生成
    ...
    const resultRender = (item: QAMessage) => {
        switch (item.type) {
            ...
            case "assistant":
                console.log(item.processLog, "sss")
                return (
                    <div className={styles["qa-answer"]}>
                        {!item.showDetails && item.isStreaming && !item.showRelated ? (
                            ...
                            <div className={styles["qa-answer-contents"]}>
                                <div className={styles["title"]}>
                                    <OutlineSparklesColorsIcon />
                                    思考
                                </div>
                                {item.processLog ? (
                                    <div className={styles["answer-content-typewriter"]}>
                                        <div className={styles["xxx"]} />
                                        {/* {item.processLog} */}
                                        这是一个知识库 这是一个知识库 这是一个知识库 这是一个知识库 这是一个知识库
                                        这是一个知识库 这是一个知识库 这是一个知识库 这是一个知识库 这是一个知识库
                                        这是一个知识库
                                        {/* </div> */}
                                        {/* <Typewriter text={item.processLog} speed={2} /> */}
                                    </div>
                                ) : (
                        ...
                        {item.showRelated && item.entries?.length ? (
                            <div className={styles["qa-details-items"]}>
                                <div className={styles["titles"]}>关联知识</div>
                        ...
                        {!item.isStreaming && item.finalAnswer?.length ? (
                            <div className={styles["qa-answer-content"]}>
                                <div className={styles["titles"]}>AI 响应</div>
