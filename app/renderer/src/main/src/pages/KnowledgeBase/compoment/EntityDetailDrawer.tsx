...
const EntityDetailDrawer: FC<EntityDetailDrawerProps> = ({
    ...
    useEffect(() => {
        if (entityDrawerDetail.visible) {
            console.log(generateKnowledgeBaseItem, entityDrawerDetail, "sss")
    ...
    const onSubmit = async () => {
        ...
        await ipcRenderer.invoke("UpdateEntity", transformData).then(() => {
        ...
        if (type === "base" && status === "preview") {
            return (
                <YakitButton type='text2'>
                    <PencilAltIcon onClick={() => setStatus("edit")} />
                </YakitButton>
            )
            ...
            <div className={styles["detail-left"]}>
                <div className={styles["detail-header"]}>
                    <div className={styles["header-buttons"]}>
                        ...
                        {/* {type === "related" ? <div className={styles["number"]}>{data?.length ?? 0}</div> : null} */}
                    </div>
