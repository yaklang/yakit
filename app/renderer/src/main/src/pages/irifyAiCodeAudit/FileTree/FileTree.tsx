...
import type { OpenFileByPathProps } from '../YakRunnerIrifyAiCodeAuditType'
...
export const FileTree: React.FC<FileTreeProps> = memo((props) => {
  ...
  useEffect(() => {
    // 滚动文件树
    emiter.on('onAiCodeAuditScrollToFileTree', onScrollToFileTreeFun)
    return () => {
      emiter.off('onAiCodeAuditScrollToFileTree', onScrollToFileTreeFun)
    }
  }, [])
  ...
  useEffect(() => {
    // 重置文件树
    emiter.on('onAiCodeAuditResetFileTree', onResetFileTreeFun)
    return () => {
      emiter.off('onAiCodeAuditResetFileTree', onResetFileTreeFun)
    }
  }, [])
  ...
  useEffect(() => {
    // 展开文件树
    emiter.on('onAiCodeAuditExpandedFileTree', onExpandedFileTreeFun)
    return () => {
      emiter.off('onAiCodeAuditExpandedFileTree', onExpandedFileTreeFun)
    }
  }, [])
...
const FileTreeNode: React.FC<FileTreeNodeProps> = memo((props) => {
  ...
  const openTernimalFun = useMemoizedFn(() => {
    if (!info.isFolder && info.parent) {
      emiter.emit('onAiCodeAuditOpenTernimal', info.parent)
    } else {
      emiter.emit('onAiCodeAuditOpenTernimal', info.path)
    }
  })
  ...
  const onPaste = useMemoizedFn(async () => {
    try {
      ...
      setCopyPath('')
      emiter.emit('onAiCodeAuditExpandedFileTree', rootPath)
  ...
  // 操作文件树（快捷键）
  useEffect(() => {
    emiter.on('onAiCodeAuditOperationFileTree', onOperationFileTreeFun)
    return () => {
      emiter.off('onAiCodeAuditOperationFileTree', onOperationFileTreeFun)
    }
  }, [])
      ...
      if (value.length !== 0 && value !== info.name) {
        ...
        if (info.isFolder) {
          if (info.parent) {
            ...
            // 此处更改布局信息
            const updatePath = (await judgeAreaExistFilesPath(areaInfo, removeFileArr)).map((item) => item.path)
  ...
  const resetCreate = useMemoizedFn(() => {
    ...
    if (info.parent) {
      ...
      if (newFolderDetail.length === 0) {
        ...
        emiter.emit('onAiCodeAuditResetFileTree', JSON.stringify({ path: info.parent }))
      }
      setMapFolderDetail(info.parent, newFolderDetail)
    }
    setFoucsedKey('')
  ...
  const onDelete = useMemoizedFn(async () => {
    ...
    emiter.emit('onAiCodeAuditDeleteInFileTree', info.path)
  })
  ...
  const onNewFile = useMemoizedFn(async (path: string) => {
    try {
      ...
      emiter.emit('onAiCodeAuditNewFileInFileTree', path)
    } catch (error) {}
  })
  ...
  const onNewFolder = useMemoizedFn(async (path: string) => {
    try {
      ...
      emiter.emit('onAiCodeAuditNewFolderInFileTree', path)
    } catch (error) {}
  })
