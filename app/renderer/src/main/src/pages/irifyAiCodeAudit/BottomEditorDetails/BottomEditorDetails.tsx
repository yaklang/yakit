...
export const BottomEditorDetails: React.FC<BottomEditorDetailsProps> = (props) => {
  ...
  const onJumpToEditor = useMemoizedFn((selections: Selection) => {
    if (activeFile?.path) {
      ...
      emiter.emit('onAiCodeAuditJumpEditorDetail', JSON.stringify(obj))
    }
  })
  ...
  useEffect(() => {
    emiter.on('onAiCodeAuditOpenBottomDetail', onOpenBottomDetailFun)
    emiter.on('onAiCodeAuditOpenTerminaDetail', onOpenTerminaDetailFun)
    return () => {
      emiter.off('onAiCodeAuditOpenBottomDetail', onOpenBottomDetailFun)
      emiter.off('onAiCodeAuditOpenTerminaDetail', onOpenTerminaDetailFun)
    }
  }, [])
  ...
  // 在终端中打开
  useEffect(() => {
    emiter.on('onAiCodeAuditOpenTernimal', onOpenTernimalFun)
    return () => {
      emiter.off('onAiCodeAuditOpenTernimal', onOpenTernimalFun)
    }
  }, [])
