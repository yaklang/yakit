export default function CreatReportScript() {
    let script:string =  `yakit.AutoInitYakit()
    loglevel($info$)
    ...
    for riskInstance = range risk.YieldRiskByCreateAt(createAt) {
        ...
        if parseBool(get(riskInstance, "IsPotential")) {
    ...
    // 风险统计展示
    reportInstance.Markdown("## 风险统计")
    reportInstance.Markdown("### 漏洞统计列表")
    ...
    for target,risks = range targetToRisks {
        ...
        for _,riskIns := range risks {
            if str.Contains(riskIns.Severity, "critical") {
                if parseBool(get(riskIns, "IsPotential")) {
