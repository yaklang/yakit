...
const AnalysisMain: React.FC<AnalysisMainProps> = React.memo((props) => {
    ...
    const getHttpFlow = useMemoizedFn(() => {
        if (!clickHttpFlow?.Id) {
            setHttpFlowRequest(firstHttpFlow ? getSafeHTTPRequest(firstHttpFlow) : "")
            return
        }
        // 是否获取Request
        let isGetRequest: boolean = true
        // 请求不为空直接使用
        if (clickHttpFlow?.RequestString) {
            isGetRequest = false
            setHttpFlowRequest(getSafeHTTPRequest(clickHttpFlow))
        }
        if (isGetRequest) {
            setHttpFlowLoading(true)
            ipcRenderer
                .invoke("GetHTTPFlowById", {Id: clickHttpFlow?.Id})
                .then((i: HTTPFlow) => {
