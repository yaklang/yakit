      ...
      if (res) {
        try {
          const { firstSizePercent, secondSizePercent } = JSONParseLog(res, {
            page: 'HTTPHistoryAnalysis',
            fun: 'HTTPFlowAnalysisYakitResizeBox',
          })
          setLastRatio({
    ...
    getRemoteValue(RemoteHistoryGV.HTTPFlowAnalysisMainYakitResizeBox).then((res) => {
      if (res) {
        try {
          const { firstSizePercent, secondSizePercent } = JSONParseLog(res, {
            page: 'HTTPHistoryAnalysis',
            fun: 'HTTPFlowAnalysisMainYakitResizeBox',
          })
          setLastRatio({
            firstRatio: firstSizePercent,
            secondRatio: secondSizePercent,
          ...
          <YakitResizeBox
            ...
            firstNode={
              <div className={styles['AnalysisMain-left']}>
                <div className={styles['rule-wrapper']} style={{ display: curBottomTab === 'rule' ? 'block' : 'none' }}>
                          ...
                          <HotCodeTemplate
                            ...
                            triggerNode={
                              <YakitButton
                                type="text"
                                size="small"
                                className={styles['hotPatch-sidebar-template-trigger']}
                              >
                                <span
                                  className={classNames(styles['hotPatch-sidebar-template-text'], 'content-ellipsis')}
                                >
                ...
                {executeStatus === 'default' || isExit ? (
                  ...
                  >
                    ...
                    <div className={styles['exec-form-item']}>
                      <span className={styles['exec-form-item-label']}>{t('AnalysisMain.data_type')}</span>
                      <YakitRadioButtons
                        value={sourceType}
                    ...
                    <div className={styles['exec-form-item']}>
                      <span className={styles['exec-form-item-label']}>{t('AnalysisMain.concurrency')}</span>
                      <YakitInputNumber
                        type="horizontal"
                        size="small"
                        value={concurrency}
                          ...
                          {matchersList.length === 0 ? (
                            ...
                            <>
                              {matchersList.map((matcherItem, index) => (
                                <div className={styles['matchers-item']} key={`matchersID:${index}`}>
                                  ...
                                  {matcherItem.SubMatchers.map((subItem, subIndex) => (
                                    <div className={styles['matchersList-item']} key={`matchersID:${subIndex}`}>
                                      ...
                                      <MatchersAndExtractorsListItemOperate
                                        onRemove={() => onRemoveMatcher(index, subIndex)}
                                        onEdit={() => onEditMatcher(index, subIndex)}
                                        popoverContent={
                                          <MatcherItem
                                            matcherItem={subItem}
                                            onEdit={() => {}}
                    ...
                    <div className={styles['exec-btn']}>
                      <YakitButton size="middle" type="primary" onClick={onStartExecute} style={{ width: 100 }}>
                  ...
                  <div className={styles['AnalysisMain-right-noDefault']}>
                    {/* 执行结果 */}
                    <div className={styles['AnalysisMain-header']}>
                      <div className={styles['AnalysisMain-header-text']}>{t('AnalysisMain.execution_result')}</div>
  ...
  return (
    <div className={styles['HttpRule-table-wrapper']}>
      <TableVirtualResize<HTTPFlowRuleData>
        ...
        extra={
          <>
            <YakitButton type="primary" onClick={exportMITMRuleExtractedData} disabled={executeStatus === 'process'}>
