...
export const HubListLocal: React.FC<HubListLocalProps> = memo((props) => {
  ...
  const updateGroupList = useMemoizedFn(() => {
    ...
    apiFetchSaveYakScriptGroupLocal({
      ...
      if (addGroup.length) {
        yakitNotify(
          'success',
          `${allChecked ? response.Total : query.IncludedScriptNames?.length}个插件已添加至“${addGroup.join(',')}”组`,
        )
      }
  ...
  const handleRemoveGroup = async (saveGroup: string[], removeGroup: string[]) => {
    ...
    if (removeGroup.length) {
      yakitNotify(
        'success',
        `${allChecked ? response.Total : query.IncludedScriptNames?.length}个插件已从“${removeGroup.join(',')}”组移除`,
      )
      fetchFilterGroup()
    }
  }
                    ...
                    <div className={styles['header-filter-tag']}>
                      {showGroupList.length <= 2 ? (
                        ...
                        >
                          ...
                          >
                            <span>
                              插件组 <span className={styles['total-style']}>{showGroupList.length}</span>
                            </span>
