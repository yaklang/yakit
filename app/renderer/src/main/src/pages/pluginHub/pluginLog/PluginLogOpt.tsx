...
export const PluginLogOpt: React.FC<PluginLogOptProps> = memo((props) => {
    ...
    const replyRoleTag = useMemo(() => {
        ...
        return null
    }, [typeInfo.key, info.parentComment])
    ...
    const additional = useMemo(() => {
        ...
        if (key === "comment") {
            ...
            return (
                <>
                    ...
                    {!!imgs && (
                        <div className={styles["comment-image-style"]}>
        ...
        if (key === "reply") {
            const {text, imgs} = pluginSupplementJSONConvertToData(info.description) || {}
            const {text: replyText, imgs: replyImgs} =
                pluginSupplementJSONConvertToData(info.parentComment?.description || "") || {}
            return (
                <>
                    <div className={styles["reply-style"]}>
                        ...
                        <div className={styles["reply-content"]}>
                            ...
                            {replyImgs && replyImgs.length > 0 && <div>{`[图片] * ${replyImgs?.length}`}</div>}
                        </div>
                    </div>
                    <div
    ...
    const showCode = useMemo(() => {
        const {key} = typeInfo
        if (["update", "checkPass", "applyMerge", "mergePass", "m ergeNoPass"].includes(key)) {
            return true
        }
        return false
    }, [typeInfo])
    ...
    const showSupplement = useMemo(() => {
        ...
        return false
    }, [userInfo.isLogin, plugin.status, info.loginIsPluginUser, info.description])
    // 是否展示回复
                ...
                <div className={styles["info-body"]}>
                    ...
                    >
                        ...
                        <div className={styles["header-operate"]}>
                            ...
                            {["comment", "reply"].includes(typeInfo.key) && (
                                <YakitButton
                                    className={styles["reply-btn"]}
                                    type='text'
                                    colors='danger'
...
// [
//     {type: "text", value: "我实名不符，贴上图片 diss 你"},
//     {
//         type: "image",
//         value: {
//             url: "https://yakit-online.oss-cn-hongkong.aliyuncs.com/img/AJBnkvdhSgGtEi6SZaLGNg==-.jpg",
//             width: 819,
//             height: 579
//         }
//     },
...
//         type: "image",
//         value: {
//             url: "https://yakit-online.oss-cn-hongkong.aliyuncs.com/img/pj6FIUTZSCCiRxesHBmA6g==-.jpg",
//             width: 1000,
//             height: 1000
//         }
//     }
