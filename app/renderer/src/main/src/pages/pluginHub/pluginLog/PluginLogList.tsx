...
export const PluginLogList: React.FC<PluginLogListProps> = memo((props) => {
    ...
    const handleCallbackMerge = useMemoizedFn((result: boolean, reason?: string) => {
        if (result) {
            httpFetchMergePluginDetail({uuid: plugin.uuid, up_log_id: logId.current})
                .then(async (res) => {
                    console.log("不合并|合并操作后的再次请求获取详情\n", res)
                })
                .catch((err) => {})
        }
        logId.current = 0
