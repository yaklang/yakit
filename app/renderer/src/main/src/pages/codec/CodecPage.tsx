...
export const CodecPage: React.FC<CodecPageProp> = (props) => {
    ...
    return (
        <Spin spinning={loading} style={{ margin: 8 }}>
            <PageHeader title={"Codec：编码与解码"}></PageHeader>
            <div style={{ width: "100%" }}>
                <Space style={{ width: "100%" }} direction={"vertical"}>
                                ...
                                if ((i.subTypes || []).length > 0) {
                                    items.push(
                                        <Popover
                                            ...
                                            content={
                                                <Space>
                                                    {(i.subTypes || []).map((subType, index) => {
                                                        return (
                                                            <Button
                                                                key={`${subType.key}-${index}`}
                                                                style={{ marginRight: 8 }}
                                                                onClick={() => {
                                                                    codec(subType.key || "")
                                                                }}
                    ...
                    <div style={{ height: 300 }}>
                        <YakEditor value={result} setValue={setResult} readOnly={true} />
                    </div>
                    <CopyToClipboard
                        text={result}
                        onCopy={(text, ok) => {
