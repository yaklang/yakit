...
export const NightingleRose: React.FC<NightingleRoseProps> = (props) => {
  ...
  useLayoutEffect(() => {
    ...
    baseOption.tooltip.formatter = (params: any) => `${params.data.name} : ${params.data.value} (${params.percent}%)`
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
        {dataSource.map((item: any) => {
          return (
            ...
            >
              <div style={{ color: 'var(--Colors-Use-Neutral-Text-1-Title)' }}>{item.key_verbose || item.key}</div>
