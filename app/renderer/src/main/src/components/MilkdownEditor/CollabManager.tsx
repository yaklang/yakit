...
import type { CollabStatus } from './MilkdownEditorType'
import { yakitNotify } from '@/utils/notification'
import type { NotepadWsRequest } from './WebsocketProvider/WebsocketProviderType'
...
export class CollabManager extends ObservableV2<CollabManagerEvents> {
  ...
  flush = async (template: string) => {
    ...
    if (!wsUrl) {
      yakitNotify('error', tOriginal('MilkdownEditor.collab.wsUrlInvalid'))
      return
    }
    this.doc?.destroy()
