...
export const getHttpFileLinkInfo: APIFunc<string, HttpFileInfoRespose> = (onlineUrl, hiddenError) => {
    return new Promise((resolve, reject) => {
        ipcRenderer
            ...
            .then(resolve)
            .catch((error) => {
                if (!hiddenError) yakitNotify("error", tOriginal("MilkdownEditor.customFile.getLinkInfoError", {error: String(error)}))
                reject(error)
            })
    })
}
...
export const getLocalFileLinkInfo: APIFunc<string, LocalFileInfoRespose> = (path, hiddenError) => {
    return new Promise((resolve, reject) => {
        ipcRenderer
            ...
            .catch((error) => {
                if (!hiddenError) yakitNotify("error", tOriginal("MilkdownEditor.customFile.getFileInfoError", {error: String(error)}))
                reject(error)
            })
    })
}
...
export const getLocalFileName: APIFunc<string, GetLocalFileTypeRespose> = (path, hiddenError) => {
    return new Promise((resolve, reject) => {
        ipcRenderer
            ...
            .then(resolve)
            .catch((error) => {
                if (!hiddenError) yakitNotify("error", tOriginal("MilkdownEditor.customFile.getLocalFileNameFailed", {error: String(error)}))
                reject(error)
            })
    })
}
