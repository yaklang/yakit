  ...
  $prose((ctx) => {
    ...
    return new Plugin({
      ...
      appendTransaction: (transactions, oldState, newState) => {
        ...
        // 不是用户真正删除行为，直接跳过
        const hasReplaceStep = transactions.some(
          (tr) => tr.steps.some((step) => step instanceof ReplaceStep) && tr.docChanged,
        )
        if (!hasReplaceStep) {
          return null
        }
        ...
        if (restoredFiles.length > 0) {
          ctx.update(deletedFileUrlsCtx, (prev) => prev.filter((item) => !restoredFiles.includes(item.fileName)))
        }
        return null
      },
    })
