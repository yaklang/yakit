...
export const convertSelectionByNode = (nodeType: NodeType, attrs: Attrs | null = null): Command => {
    return (state, dispatch) => {
        try {
            ...
            yakitNotify("error", tOriginal("MilkdownEditor.utils.convertSelectionFailed", {nodeType: nodeType.name, error: String(error)}))
            return false
        }
    }
}
...
export const uploadFileInMilkdown = (
    ...
    handleOpenFileSystemDialog({title: tOriginal("MilkdownEditor.utils.selectFile"), properties: ["openFile"]}).then((data) => {
        const filesLength = data.filePaths.length
        if (filesLength) {
            const path = data.filePaths[0].replace(/\\/g, "\\")
            getLocalFileLinkInfo(path).then((res) => {
                if (res.size > FileMaxSize) {
                    yakitNotify("error", tOriginal("MilkdownEditor.utils.fileTooLarge"))
                    return
                }
                ...
                if (imgTypes.includes(fileType)) {
                    if (res.size > ImgMaxSize) {
                        yakitNotify("error", tOriginal("MilkdownEditor.utils.imageTooLarge"))
                        return
                    }
                    httpUploadImgPath({path, type, filedHash: notepadHash})
                        ...
                        .catch((e) => {
                            yakitNotify("error", tOriginal("MilkdownEditor.utils.uploadImageFailed", {error: String(e)}))
                        })
                } else {
                    action(
