...
export const convertSelectionByNode = (nodeType: NodeType, attrs: Attrs | null = null): Command => {
  return (state, dispatch) => {
    try {
      ...
      yakitNotify(
        'error',
        tOriginal('MilkdownEditor.utils.convertSelectionFailed', { nodeType: nodeType.name, error: String(error) }),
      )
      return false
    }
  }
...
export const uploadFileInMilkdown = (
  ...
  handleOpenFileSystemDialog({ title: tOriginal('MilkdownEditor.utils.selectFile'), properties: ['openFile'] }).then(
    (data) => {
