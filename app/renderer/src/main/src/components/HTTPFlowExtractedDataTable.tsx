    ...
    const update = useMemoizedFn((page?: number, limit?: number, HTTPFlowHash?: string) => {
        ...
        ipcRenderer
            ...
            .then((r: QueryGeneralResponse<HTTPFlowExtractedData>) => {
                setData(r.Data)
                props.onSetHighLightText(r.Data.map((i) => ({startOffset: i.Index, highlightLength: i.Length, hoverVal: i.RuleName})))
                ...
                columns={[
                    {
                        ...
                        render: (i: HTTPFlowExtractedData) => (
                            <div className='content-ellipsis' style={{width: 100, overflow: "hidden"}}>
                                {i.RuleName}
                            </div>
                        ),
                        width: 100
                    },
                    {
                        title: "规则数据",
                        render: (i: HTTPFlowExtractedData) => (
                            <div className='content-ellipsis' style={{width: 200, overflow: "hidden"}}>
                                {i.Data.slice(0, 50)}
                            </div>
                        ),
                        width: 210
