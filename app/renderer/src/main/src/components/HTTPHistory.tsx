...
const getHistoryTabs = (pageType: HTTPHistorySourcePageType): YakitTabsProps[] => {
  const tabs: YakitTabsProps[] = [
    {
...
export const HistoryTab: YakitTabsProps[] = getHistoryTabs('History')

interface MITMExtractAggregateRow {
  RuleVerbose: string
  DisplayData: string
  HitCount: number
...
interface MITMExtractAggregatePanelProps {
  runtimeId?: string
  httpFlowQueryParams: string
  onSetFilterRows: (rows: MitmExtractAggregateFlowFilterRow[]) => void
  onSetSelectedTraceIds: (ids: string[]) => void
}

const MITMExtractAggregatePanel: React.FC<MITMExtractAggregatePanelProps> = React.memo((props) => {
  const { runtimeId, httpFlowQueryParams, onSetFilterRows, onSetSelectedTraceIds } = props
  const [loading, setLoading] = useState(false)
  const [rows, setRows] = useState<MITMExtractAggregateRow[]>([])
  ...
  const extractKey = useMemoizedFn((row: MITMExtractAggregateRow) => `${row.RuleVerbose}\u0000${row.DisplayData}`)

  const hasHttpFlowFilter = useMemoizedFn((f: YakQueryHTTPFlowRequest | undefined) => {
    if (!f) return false
    return Object.entries(f).some(([k, v]) => {
      if (v === undefined || v === null || v === '') return false
      if (Array.isArray(v) && v.length === 0) return false
      if (typeof v === 'boolean' && v === false) return false
      if (k === 'SourceType' || k === 'Full' || k === 'WithPayload') return false
      return true
    })
  })

  const parseFlowFilter = useMemoizedFn(() => {
    const raw = (httpFlowQueryParams || '').trim()
    if (!raw) return undefined
    try {
      const parsed = JSONParseLog(raw, { page: 'HTTPHistory', fun: 'MITMExtractAggregatePanel' }) || {}
      delete parsed.Pagination
      delete parsed.AfterId
      delete parsed.BeforeId
      return stripMitmAggregateHttpFlowLiveWindow(parsed as YakQueryHTTPFlowRequest)
    } catch (error) {
      return undefined
    }
  })
  ...
  const refreshData = useMemoizedFn(async () => {
    ...
    try {
      const req: Record<string, any> = {
        Pagination: { Page: page, Limit: pageSize, OrderBy: 'hit_count', Order: 'desc' },
        IncludeDistinctRuleGroups: true,
      }
      if (runtimeId) req.RuntimeId = runtimeId
      if (selectedGroup) req.RuleGroup = selectedGroup
      const flowFilter = parseFlowFilter()
      if (hasHttpFlowFilter(flowFilter)) {
      ...
      const {
        rows: data,
        total: aggTotal,
        distinctRuleGroups: groups,
      } = normalizeQueryMITMExtractedAggregateResponse(rsp)
      setRows(data as MITMExtractAggregateRow[])
      setTotal(aggTotal)
      setGroupList([...groups].sort((a, b) => a.localeCompare(b)))
      setSelectedRowKeys((prev) => prev.filter((k) => data.some((r) => extractKey(r) === k)))
    } finally {
      setLoading(false)
    }
  })
  ...
  useDebounceEffect(
    ...
    [page, selectedGroup, runtimeId, httpFlowQueryParams],
    { wait: 350, leading: true },
  )

  const selectedRows = useMemo(
    () => rows.filter((item) => selectedRowKeys.includes(extractKey(item))),
    [rows, selectedRowKeys],
  )

  useEffect(() => {
    onSetFilterRows(selectedRows.map((item) => ({ RuleVerbose: item.RuleVerbose, DisplayData: item.DisplayData })))
    if (selectedRows.length > 0) {
      onSetSelectedTraceIds([])
    }
  }, [selectedRows, onSetFilterRows, onSetSelectedTraceIds])

  const pageCount = Math.max(1, Math.ceil(total / pageSize))
  const toggleSelect = useMemoizedFn((row: MITMExtractAggregateRow) => {
  ...
  return (
    <div className={styles['aggregate-panel']}>
      <div className={styles['aggregate-toolbar']}>
        <div className={styles['aggregate-title']}>Rule | Data</div>
        <div className={styles['aggregate-toolbar-actions']}>
          <YakitButton
            type="text2"
            size="small"
            icon={<RefreshIcon />}
      ...
      {groupList.length > 0 ? (
        <div className={styles['aggregate-group-bar']}>
          {groupList.map((g) => (
            <YakitButton
              key={g}
              size="small"
              type={selectedGroup === g ? 'primary' : 'text2'}
              onClick={() => {
      ...
      <div className={styles['aggregate-list']}>
        {loading ? (
          ...
          rows.map((record) => {
            ...
            return (
              <div
                key={key}
                className={classNames(styles['aggregate-item'], checked && styles['aggregate-item-active'])}
              >
                <YakitCheckbox
                  checked={checked}
                ...
                />
                <div
                  className={styles['aggregate-item-name']}
                  title={`${record.RuleVerbose} | ${record.DisplayData}`}
                  onClick={() => {
                    const ids = Array.isArray(record.SampleTraceIds) ? record.SampleTraceIds : []
                    onSetSelectedTraceIds(ids)
                  }}
                >
                  <span className={styles['aggregate-item-rule']}>{record.RuleVerbose}</span>
                  <span className={styles['aggregate-item-sep']}>|</span>
                  <span className={styles['aggregate-item-data']}>{record.DisplayData}</span>
                </div>
              </div>
            )
          })
      ...
      <div className={styles['aggregate-footer']}>
        <span>{`第 ${page}/${pageCount} 页，共 ${total} 条`}</span>
        <div className={styles['aggregate-footer-actions']}>
          <YakitButton
            size="small"
            type="text2"
            disabled={page <= 1}
...
const HTTPHistoryInner: React.FC<HTTPHistoryProp> = (props) => {
  ...
  // #region 左侧tab
  const historyTabs = useMemo(() => getHistoryTabs(pageType), [pageType])
  ...
  useEffect(() => {
    if (!historyTabs.some((item) => item.value === activeKey)) {
      onActiveKey('web-tree')
    }
  }, [historyTabs, activeKey])
  ...
  const [aggregateQueryparams, setAggregateQueryparams] = useState<string>('')
  const [mitmAggregateFilterRows, setMitmAggregateFilterRows] = useState<MitmExtractAggregateFlowFilterRow[]>([])
  const [mitmAggregateSelectedTraceIds, setMitmAggregateSelectedTraceIds] = useState<string[]>([])
  ...
  const onQueryParams = useMemoizedFn((queryParams, execFlag) => {
    try {
      ...
      setAggregateQueryparams(queryParams || '')
      if (pageType === 'MITM') {
        emiter.emit(
  ...
  const mergedMITMParams = useMemo(() => {
    if (pageType !== 'MITM') return historyProps.params
    const merged: YakQueryHTTPFlowRequest = {
      ...(historyProps.params || {}),
    }
    if (mitmAggregateFilterRows.length > 0) {
      merged.MitmExtractAggregateFilterRows = mitmAggregateFilterRows
      delete merged.HiddenIndex
      return merged
    }
    // 与独立聚合页一致：传空数组才能覆盖 HTTPFlowTable 内仍保留的旧 MitmExtractAggregateFilterRows
    merged.MitmExtractAggregateFilterRows = []
    if (mitmAggregateSelectedTraceIds.length > 0) {
      merged.HiddenIndex = mitmAggregateSelectedTraceIds
    } else {
      delete merged.HiddenIndex
    }
    return merged
  }, [pageType, historyProps.params, mitmAggregateFilterRows, mitmAggregateSelectedTraceIds])
        ...
        firstNode={() => (
          <div className={styles['hTTPHistory-left']}>
            ...
            <div className={styles['tab-content']}>
              ...
              <div
                className={styles['aggregate-wrapper']}
                style={{ display: activeKey === 'rule-data' ? 'block' : 'none' }}
              >
                {pageType === 'MITM' ? (
                  <MITMExtractAggregatePanel
                    runtimeId={historyProps.params?.RuntimeId}
                    httpFlowQueryParams={aggregateQueryparams}
                    onSetFilterRows={setMitmAggregateFilterRows}
                    onSetSelectedTraceIds={setMitmAggregateSelectedTraceIds}
                  />
                ) : (
                  <YakitEmpty />
                )}
