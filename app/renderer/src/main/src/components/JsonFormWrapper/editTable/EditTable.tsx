        ...
        case 'textarea':
          return (
            <YakitInput.TextArea
              ...
              rows={3}
              placeholder={t('JsonFormWrapper.EditTable.enterField', {
                field: typeof title === 'object' ? t('JsonFormWrapper.EditTable.field') : title,
              })}
              onFocus={onFocus}
            />
          )
      ...
      {editing ? (
        <Form.Item
          ...
          rules={[
            {
              required,
              message: t('JsonFormWrapper.EditTable.fillField', {
                field: typeof title === 'object' ? t('JsonFormWrapper.EditTable.field') : title,
              }),
            },
          ]}
...
export const EditTable: React.FC<EditTableProps> = (props) => {
  ...
  useEffect(() => {
    try {
      ...
      failed(t('JsonFormWrapper.EditTable.parseTableFailed', { error: String(error) }))
    }
  ...
  const onCopy = useMemoizedFn(async (record: Item) => {
    ...
    if (record._id === editingId) {
      try {
        ...
        warn(t('JsonFormWrapper.EditTable.currentRowInvalid'))
      }
    } else {
      const newRecord = {
        ...record,
  ...
  const addCell = useMemoizedFn(async () => {
    if (typeof maxItems === 'number' && data.length >= maxItems) {
      warn(t('JsonFormWrapper.EditTable.maxItemsReached', { maxItems }))
      return
    }
