    ...
    <div className={styles['demo-packet-table-box']}>
      <DemoVirtualTable<TrafficPacket>
        ...
        columns={[
          ...
          {
            headerTitle: t('playground.DemoPacketTable.source'),
            key: 'source',
            width: 160,
            colRender: (i) => i.NetworkEndpointIPSrc + ':' + i.TransportEndpointPortSrc,
          },
          {
            headerTitle: t('playground.DemoPacketTable.destination'),
            key: 'destination',
            width: 160,
            colRender: (i) => i.NetworkEndpointIPDst + ':' + i.TransportEndpointPortDst,
          },
          {
            headerTitle: t('playground.DemoPacketTable.protocol'),
            key: 'Protocol',
            width: 160,
            colRender: (i) => i.Protocol,
          },
          {
            headerTitle: t('playground.DemoPacketTable.length'),
            key: 'Length',
            width: 80,
            colRender: (i) => (i.Raw || []).length,
          },
          { headerTitle: t('playground.DemoPacketTable.info'), key: 'Info', width: 160, colRender: (i) => i.Info },
        ]}
        rowClick={(data) => {
          onSelectFun(data)
        }}
