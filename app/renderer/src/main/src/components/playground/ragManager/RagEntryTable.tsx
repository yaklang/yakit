...
export const RagEntryTable: React.FC<RagEntryTableProps> = ({ selectedCollection, onRefresh }) => {
  ...
  const fetchEntries = useMemoizedFn(async () => {
    ...
    try {
      ...
      failed(t('playground.RagEntryTable.fetchFailed', { error: String(error) }))
      setEntries([])
      setTotal(0)
    } finally {
      setLoading(false)
  ...
  const handleViewDetail = useMemoizedFn(async (entry: VectorStoreEntry) => {
    ...
    if (entry.ID) {
      try {
        ...
        failed(t('playground.RagEntryTable.fetchDocumentFailed', { error: String(error) }))
  ...
  const columns: ColumnsTypeProps[] = [
    ...
    },
    {
      ...
      dataKey: 'UID',
      width: 120,
      render: (text, item: VectorStoreEntry) => <div className={styles['document-id-cell']}>{item.UID || '-'}</div>,
    },
    {
      title: t('playground.RagEntryTable.contentPreview'),
      dataKey: 'Content',
      width: 300,
    ...
    {
      title: t('playground.RagEntryTable.metadata'),
      dataKey: 'Metadata',
      width: 200,
      render: (text, item: VectorStoreEntry) => (
        <div className={styles['metadata-cell']}>
          {item.Metadata ? (
            ...
            <span className={styles['no-metadata']}>{t('playground.RagEntryTable.noMetadata')}</span>
          )}
        </div>
      ),
    },
    {
      title: t('playground.RagEntryTable.vectorDimension'),
    ...
    {
      title: t('playground.RagEntryTable.createdAt'),
      dataKey: 'CreatedAt',
      width: 150,
  ...
  if (!selectedCollection) {
    return (
      <div className={styles['rag-entry-table']}>
        <AutoCard title={t('playground.RagEntryTable.title')} bodyStyle={{ padding: 24 }}>
          <YakitEmpty description={t('playground.RagEntryTable.selectCollectionFirst')} />
        </AutoCard>
      </div>
    )
  }
  ...
  return (
    <div className={styles['rag-entry-table']}>
      <AutoCard
        title={t('playground.RagEntryTable.titleWithCollection', { name: selectedCollection.Name })}
        size="small"
        bodyStyle={{ padding: 0 }}
        extra={
          <Space>
            <YakitInput.Search
              placeholder={t('playground.RagEntryTable.searchPlaceholder')}
              value={searchKeyword}
        ...
        {selectedEntryDetail && (
          <div className={styles['entry-detail']}>
            <div className={styles['detail-section']}>
              <h4>{t('playground.RagEntryTable.basicInfo')}</h4>
              <div className={styles['detail-grid']}>
                <div className={styles['detail-row']}>
                  <span className={styles['label']}>{t('playground.RagEntryTable.entryId')}:</span>
                  <span className={styles['value']}>{selectedEntryDetail.ID}</span>
                </div>
                <div className={styles['detail-row']}>
                  <span className={styles['label']}>UID:</span>
                  <span className={styles['value']}>
                    {selectedEntryDetail.UID || t('playground.RagEntryTable.none')}
                  </span>
                </div>
                <div className={styles['detail-row']}>
                  <span className={styles['label']}>{t('playground.RagEntryTable.collectionId')}:</span>
                  <span className={styles['value']}>{selectedCollection?.ID}</span>
                </div>
                <div className={styles['detail-row']}>
                  <span className={styles['label']}>{t('playground.RagEntryTable.vectorDimension')}:</span>
                  <span className={styles['value']}>
                    {selectedEntryDetail.Embedding ? selectedEntryDetail.Embedding.length : 0}
                  </span>
                </div>
                <div className={styles['detail-row']}>
                  <span className={styles['label']}>{t('playground.RagEntryTable.contentLength')}:</span>
                  <span className={styles['value']}>
                    {selectedEntryDetail.Content ? selectedEntryDetail.Content.length : 0}{' '}
                    {t('playground.RagEntryTable.characters')}
                  </span>
                </div>
                <div className={styles['detail-row']}>
                  <span className={styles['label']}>{t('playground.RagEntryTable.metadataLength')}:</span>
                  <span className={styles['value']}>
                    {selectedEntryDetail.Metadata ? selectedEntryDetail.Metadata.length : 0}{' '}
                    {t('playground.RagEntryTable.characters')}
                  </span>
                </div>
              </div>
            ...
            {selectedEntryDetail.Content && (
              <div className={styles['detail-section']}>
                <h4>{t('playground.RagEntryTable.contentPreview')}</h4>
            ...
            {entryDocument && (
              <div className={styles['detail-section']}>
                <h4>{t('playground.RagEntryTable.relatedKnowledgeEntry')}</h4>
                <div className={styles['document-display']}>
                  {entryDocument.KnowledgeTitle && (
                    <div>
                      <strong>{t('playground.RagEntryTable.titleLabel')}:</strong> {entryDocument.KnowledgeTitle}
                    </div>
                  )}
                  {entryDocument.KnowledgeType && (
                    <div>
                      <strong>{t('playground.RagEntryTable.typeLabel')}:</strong> {entryDocument.KnowledgeType}
                    </div>
                  )}
                  {entryDocument.Summary && (
                    <div>
                      <strong>{t('playground.RagEntryTable.summaryLabel')}:</strong> {entryDocument.Summary}
                    </div>
                  )}
                  {entryDocument.KnowledgeDetails && (
                    <div>
                      <strong>{t('playground.RagEntryTable.detailsLabel')}:</strong>
                      <br />
                      {entryDocument.KnowledgeDetails}
                    </div>
                  )}
                  {entryDocument.Keywords && entryDocument.Keywords.length > 0 && (
                    <div>
                      <strong>{t('playground.RagEntryTable.keywordsLabel')}:</strong>{' '}
                      {entryDocument.Keywords.join(', ')}
                    </div>
                  )}
                  {entryDocument.ImportanceScore !== undefined && (
                    <div>
                      <strong>{t('playground.RagEntryTable.importanceLabel')}:</strong> {entryDocument.ImportanceScore}
                    </div>
                  )}
                  {entryDocument.SourcePage && (
                    <div>
                      <strong>{t('playground.RagEntryTable.sourcePageLabel')}:</strong> {entryDocument.SourcePage}
                    </div>
                  )}
                  {entryDocument.PotentialQuestions && entryDocument.PotentialQuestions.length > 0 && (
                    <div>
                      <strong>{t('playground.RagEntryTable.potentialQuestionsLabel')}:</strong>{' '}
            ...
            {selectedEntryDetail.Metadata && (
              <div className={styles['detail-section']}>
                <h4>{t('playground.RagEntryTable.metadata')}</h4>
            ...
            {selectedEntryDetail.Embedding && selectedEntryDetail.Embedding.length > 0 && (
              <div className={styles['detail-section']}>
                <h4>{t('playground.RagEntryTable.vectorDataFirst20')}</h4>
                <div className={styles['vector-display']}>
                  ...
                  {selectedEntryDetail.Embedding.length > 20 && (
                    <YakitTag size="small" color="blue">
                      {t('playground.RagEntryTable.moreDimensions', {
                        count: selectedEntryDetail.Embedding.length - 20,
                      })}
                    </YakitTag>
                  )}
                </div>
