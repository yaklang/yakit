      ...
      <YakitResizeBox
        firstNode={
          <AutoCard
            title={
              <DemoItemSelectOne
                label={t('playground.SpaceEngineOperator.engine')}
                ...
                data={[
                  ...
                  { label: t('playground.SpaceEngineOperator.zone'), value: 'zone' },
                  { label: t('playground.SpaceEngineOperator.unset'), value: '' },
                ]}
          ...
          >
              ...
              <Alert
                ...
                description={
                  <div>
                    ...
                    {status.Info}
                    {status.Used > 0 && (
                      <YakitTag>{t('playground.SpaceEngineOperator.usedQuota', { count: status.Used })}</YakitTag>
                    )}
                    {status.Remain > 0 && (
                      <YakitTag>
                        {t('playground.SpaceEngineOperator.remainingQuota', { count: status.Remain })}
                      </YakitTag>
                    )}
            ...
            {!noEngine && (
              <Form layout={'vertical'} onSubmitCapture={(e) => e.preventDefault()} disabled={params.Type === ''}>
                <Form.Item label={t('playground.SpaceEngineOperator.searchCondition')}>
                  <YakEditor
                ...
                <DemoItemSwitch
                  label={t('playground.SpaceEngineOperator.scan')}
                  value={params.ScanBeforeSave}
                  setValue={(i) => setParams({ ...params, ScanBeforeSave: i })}
                />
                <InputInteger
                  label={t('playground.SpaceEngineOperator.maxPage')}
                  setValue={(MaxPage) => setParams({ ...params, MaxPage })}
                  value={params.MaxPage}
                  min={1}
                />
                <InputInteger
                  label={t('playground.SpaceEngineOperator.maxRecord')}
                  setValue={(MaxRecord) => setParams({ ...params, MaxRecord })}
                  value={params.MaxRecord}
                  min={1}
                />
                <InputInteger
                  label={t('playground.SpaceEngineOperator.randomDelay')}
                  setValue={(RandomDelay) => setParams({ ...params, RandomDelay })}
                  value={params.RandomDelay || 0}
                  min={0}
                />
                <InputInteger
                  label={t('playground.SpaceEngineOperator.retryTimes')}
                  setValue={(RetryTimes) => setParams({ ...params, RetryTimes })}
