...
export const KnowledgeBaseList: React.FC<KnowledgeBaseListProps> = ({
  ...
  const fetchKnowledgeBases = useMemoizedFn(async (resetPage = false) => {
    ...
    try {
      ...
      failed(t('playground.KnowledgeBaseList.fetchFailed', { error: String(error) }))
      setKnowledgeBases([])
      setTotal(0)
    } finally {
      setLoading(false)
  ...
  const handleEmbedKnowledgeBase = useMemoizedFn(async (kb: KnowledgeBase) => {
    ...
    try {
        ...
        (prev) =>
          new Map(
            prev.set(streamKey, {
              token: '',
              loading: true,
              progress: t('playground.KnowledgeBaseList.indexingInProgress'),
            }),
          ),
      )
      ...
      success(t('playground.KnowledgeBaseList.indexBuildSuccess'))
    } catch (error) {
      setEmbedStreams((prev) => {
        const newMap = new Map(prev)
        newMap.delete(streamKey)
      ...
      failed(t('playground.KnowledgeBaseList.indexBuildFailed', { error: String(error) }))
    }
  })
  ...
  const handleCancelEmbed = useMemoizedFn(async (streamKey: string) => {
    ...
    yakitNotify('info', t('playground.KnowledgeBaseList.indexBuildCancelled'))
  })
  ...
  const handleCreate = useMemoizedFn(async (values: KnowledgeBaseFormData) => {
    try {
      ...
      failed(t('playground.KnowledgeBaseList.createFailed', { error: String(error) }))
    }
  })
  ...
  const handleUpdate = useMemoizedFn(async (values: KnowledgeBaseFormData) => {
    ...
    try {
      ...
      success(t('playground.KnowledgeBaseList.updateSuccess'))
      setModalVisible(false)
      ...
      failed(t('playground.KnowledgeBaseList.updateFailed', { error: String(error) }))
    }
  })
  ...
  const handleDelete = useMemoizedFn(async (kb: KnowledgeBase) => {
    try {
      ...
      failed(t('playground.KnowledgeBaseList.deleteFailed', { error: String(error) }))
    }
  })
  ...
  return (
    <div className={styles['knowledge-base-list']}>
      <AutoCard
        title={t('playground.KnowledgeBaseList.title')}
        size="small"
        bodyStyle={{ padding: 12 }}
        extra={
          <Space>
            <YakitInput.Search
              placeholder={t('playground.KnowledgeBaseList.searchPlaceholder')}
              value={searchKeyword}
      ...
      >
        <YakitSpin spinning={loading}>
          {knowledgeBases.length === 0 ? (
            <YakitEmpty description={t('playground.KnowledgeBaseList.empty')} />
          ) : (
                ...
                >
                  <div className={styles['kb-info']}>
                    ...
                    <div className={styles['kb-description']}>
                      {kb.KnowledgeBaseDescription || t('YakitEmpty.noDescription')}
                    </div>
                    <div className={styles['kb-type']}>
                      {t('playground.KnowledgeBaseList.type')}: {kb.KnowledgeBaseType}
                    </div>
                  </div>
                      ...
                      if (embedStatus?.loading) {
                        return (
                          <div className={styles['embed-progress']}>
                            ...
                            <span className={styles['progress-text']}>{embedStatus.progress}</span>
                            <YakitButton
                              type="text2"
                              size="small"
                              onClick={(e) => {
                      ...
                      return (
                        <>
                          <YakitButton
                            ...
                            title={t('playground.KnowledgeBaseList.buildIndex')}
                          />
                          {onOpenQA && (
                            <YakitButton
                              type="text2"
                              ...
                              title={t('playground.KnowledgeBaseList.aiQa')}
                            />
                          )}
                          <YakitButton
                            type="text2"
                          ...
                          />
                          <YakitPopconfirm
                            title={t('playground.KnowledgeBaseList.deleteConfirm')}
                            onConfirm={(e) => {
                              e?.stopPropagation()
                              handleDelete(kb)
                            }}
            ...
            <Pagination
              ...
              showTotal={(total) => (
                <div style={{ color: 'var(--Colors-Use-Neutral-Text-1-Title)' }}>
                  {t('playground.KnowledgeBaseList.totalRecords', { count: total })}
                </div>
              )}
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          <Form.Item
            label={t('playground.KnowledgeBaseList.knowledgeBaseName')}
            name="KnowledgeBaseName"
            rules={[{ required: true, message: t('playground.KnowledgeBaseList.enterKnowledgeBaseName') }]}
          >
            <YakitInput placeholder={t('playground.KnowledgeBaseList.enterKnowledgeBaseName')} />
          </Form.Item>
          <Form.Item label={t('playground.KnowledgeBaseList.knowledgeBaseDescription')} name="KnowledgeBaseDescription">
            <YakitInput.TextArea
              placeholder={t('playground.KnowledgeBaseList.enterKnowledgeBaseDescription')}
              rows={3}
              maxLength={500}
              showCount
            />
          ...
          <Form.Item
            label={t('playground.KnowledgeBaseList.knowledgeBaseType')}
            name="KnowledgeBaseType"
            rules={[{ required: true, message: t('playground.KnowledgeBaseList.enterKnowledgeBaseType') }]}
          >
            <YakitInput placeholder={t('playground.KnowledgeBaseList.enterKnowledgeBaseType')} />
          </Form.Item>
        </Form>
      </YakitModal>
    </div>
