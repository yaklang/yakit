...
export const KnowledgeEntryTable: React.FC<KnowledgeEntryTableProps> = ({ knowledgeBase, onRefresh }) => {
  ...
  const handleCreate = useMemoizedFn(async (values: KnowledgeEntryFormData) => {
    ...
    try {
      ...
      failed(t('playground.KnowledgeEntryTable.createFailed', { error: String(error) }))
    }
  })
  ...
  const handleUpdate = useMemoizedFn(async (values: KnowledgeEntryFormData) => {
    ...
    try {
      ...
      failed(t('playground.KnowledgeEntryTable.updateFailed', { error: String(error) }))
    }
  })
  ...
  const handleDelete = useMemoizedFn(async (entry: KnowledgeBaseEntry) => {
    try {
      ...
      success(t('playground.KnowledgeEntryTable.deleteSuccess'))
      searchEntries()
      onRefresh()
    } catch (error) {
      failed(t('playground.KnowledgeEntryTable.deleteFailed', { error: String(error) }))
    }
  })
  ...
  const handleCreateIndex = useMemoizedFn(async (entry: KnowledgeBaseEntry) => {
    try {
      ...
      success(t('playground.KnowledgeEntryTable.createIndexSuccess', { title: entry.KnowledgeTitle }))
    } catch (error) {
      failed(t('playground.KnowledgeEntryTable.createIndexFailed', { error: String(error) }))
    } finally {
  ...
  const columns: ColumnsTypeProps[] = [
    ...
    {
      title: t('playground.KnowledgeEntryTable.title'),
      dataKey: 'KnowledgeTitle',
      width: 200,
      render: (text, item: KnowledgeBaseEntry) => <div className={styles['title-cell']}>{item.KnowledgeTitle}</div>,
    },
    {
      title: t('playground.KnowledgeEntryTable.type'),
      dataKey: 'KnowledgeType',
      width: 120,
      render: (text, item: KnowledgeBaseEntry) => <YakitTag>{item.KnowledgeType}</YakitTag>,
    },
    {
      title: t('playground.KnowledgeEntryTable.importance'),
    ...
    {
      title: t('playground.KnowledgeEntryTable.keywords'),
      dataKey: 'Keywords',
      width: 200,
    ...
    {
      title: t('playground.KnowledgeEntryTable.summary'),
      dataKey: 'Summary',
      width: 300,
      render: (text, item: KnowledgeBaseEntry) => (
    ...
    {
      title: t('playground.KnowledgeEntryTable.sourcePage'),
      dataKey: 'SourcePage',
      width: 80,
        ...
        <Space>
          {indexingEntries.has(item.ID) ? (
            <YakitButton type="text2" size="small" loading={true} title="正在创建索引...">
              {t('playground.KnowledgeEntryTable.indexing')}
            </YakitButton>
          ) : (
            <YakitButton
              type="text2"
            ...
            >
              {t('playground.KnowledgeEntryTable.index')}
            </YakitButton>
          )}
          <YakitButton
            type="text2"
          ...
          />
          <YakitPopconfirm
            title={t('playground.KnowledgeEntryTable.deleteConfirm')}
    ...
    return (
      <div className={styles['knowledge-entry-table']}>
        <AutoCard title="知识条目" bodyStyle={{ padding: 24 }}>
          <YakitEmpty description={t('playground.KnowledgeEntryTable.selectKnowledgeBaseFirst')} />
        </AutoCard>
      </div>
    )
  }
  ...
  return (
    <div className={styles['knowledge-entry-table']}>
      <AutoCard
        title={t('playground.KnowledgeEntryTable.titleWithBase', { name: knowledgeBase.KnowledgeBaseName })}
        size="small"
        bodyStyle={{ padding: 0 }}
        extra={
          <Space>
            <YakitInput
              placeholder={t('playground.KnowledgeEntryTable.searchPlaceholder')}
              value={searchKeyword}
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          <Form.Item
            label={t('playground.KnowledgeEntryTable.knowledgeTitle')}
            name="KnowledgeTitle"
            rules={[{ required: true, message: t('playground.KnowledgeEntryTable.enterKnowledgeTitle') }]}
          >
            <YakitInput placeholder={t('playground.KnowledgeEntryTable.enterKnowledgeTitle')} />
          </Form.Item>

          <Form.Item
            label={t('playground.KnowledgeEntryTable.knowledgeType')}
            name="KnowledgeType"
            rules={[{ required: true, message: t('playground.KnowledgeEntryTable.enterKnowledgeType') }]}
          >
            <YakitInput placeholder={t('playground.KnowledgeEntryTable.enterKnowledgeType')} />
          </Form.Item>

          <Form.Item
            label={t('playground.KnowledgeEntryTable.importanceScore')}
            name="ImportanceScore"
            rules={[{ required: true, message: t('playground.KnowledgeEntryTable.enterImportanceScore') }]}
          >
            <YakitInputNumber min={1} max={10} style={{ width: '100%' }} />
          </Form.Item>

          <Form.Item label={t('playground.KnowledgeEntryTable.keywords')} name="Keywords">
          ...
          <Form.Item
            label={t('playground.KnowledgeEntryTable.knowledgeDetails')}
            name="KnowledgeDetails"
            rules={[{ required: true, message: t('playground.KnowledgeEntryTable.enterKnowledgeDetails') }]}
          >
            <YakitInput.TextArea
              placeholder={t('playground.KnowledgeEntryTable.enterKnowledgeDetails')}
              rows={6}
              maxLength={5000}
              showCount
            />
          ...
          <Form.Item label={t('playground.KnowledgeEntryTable.summary')} name="Summary">
            <YakitInput.TextArea
              placeholder={t('playground.KnowledgeEntryTable.enterSummary')}
              rows={3}
              maxLength={500}
              showCount
            />
