...
export const KnowledgeBaseQA: React.FC<KnowledgeBaseQAProps> = ({ knowledgeBase, queryAllCollectionsDefault }) => {
  ...
  const handleAsk = useMemoizedFn(async () => {
    ...
    const assistantMessage: QAMessage = {
      id: generateId(),
      type: 'assistant',
      content: t('playground.KnowledgeBaseQA.thinking') + '\n',
      timestamp: Date.now(),
      entries: [],
      isStreaming: true,
      // @ts-ignore 扩展字段
              ...
              <div className={styles['entry-content']}>
                {entry.Summary && (
                  <div className={styles['entry-summary']}>
                    <strong>{t('playground.KnowledgeBaseQA.summary')}:</strong> {entry.Summary}
                  </div>
                )}
                {entry.KnowledgeDetails && (
                  <div className={styles['entry-details']}>
                    <strong>{t('playground.KnowledgeBaseQA.details')}:</strong> {entry.KnowledgeDetails}
                  </div>
                )}
                {entry.Keywords && entry.Keywords.length > 0 && (
                  <div className={styles['entry-keywords']}>
                    <strong>{t('playground.KnowledgeBaseQA.keywords')}:</strong>
                    <Space size={4} wrap>
                ...
                {entry.SourcePage && (
                  <div className={styles['entry-source']}>
                    <strong>{t('playground.KnowledgeBaseQA.sourcePage')}:</strong> {entry.SourcePage}
                  </div>
                )}
              </div>
            </Panel>
    ...
    <div className={styles['knowledge-base-qa']}>
      ...
      {!knowledgeBase && !queryAllCollections ? (
        <div className={styles['no-kb-selected']}>
          <YakitEmpty description={t('playground.KnowledgeBaseQA.selectKnowledgeBaseFirst')} />
        </div>
      ) : (
        <>
          {/* 对话区域 */}
          <div className={styles['messages-container']}>
            {messages.length === 0 ? (
              <div className={styles['welcome-message']}>
                <YakitEmpty
                  description={t('playground.KnowledgeBaseQA.startChat', {
                    name: knowledgeBase
              ...
              <div className={styles['messages-list']}>
                {messages.map((msg) => (
                  <div key={msg.id} className={`${styles['message']} ${styles[`message-${msg.type}`]}`}>
                    <div className={`${styles['message-content']} ${msg.showRelated ? styles['show-related'] : ''}`}>
                      ...
                      {msg.type === 'assistant' && msg.finalAnswer && (
                        <div className={styles['message-actions']}>
                          ...
                          >
                            {msg.showDetails
                              ? t('playground.KnowledgeBaseQA.hideDetails')
                              : t('playground.KnowledgeBaseQA.viewDetails')}
                          </YakitButton>
                          {msg.entries && msg.entries.length > 0 && (
                            ...
                            >
                              {msg.showRelated
                                ? t('playground.KnowledgeBaseQA.hide')
                                : t('playground.KnowledgeBaseQA.relatedKnowledgeToggle')}
                            </YakitButton>
                          )}
                        </div>
                      )}
                    ...
                    <div className={styles['message-time']}>{new Date(msg.timestamp).toLocaleTimeString()}</div>
                  </div>
                ))}
            ...
            <div className={styles['config-row']}>
              ...
              >
                <YakitSelect.Option value="hypothetical_answer">
                  {t('playground.KnowledgeBaseQA.hypotheticalAnswer')}
                </YakitSelect.Option>
                <YakitSelect.Option value="generalize_query">
                  {t('playground.KnowledgeBaseQA.generalizeQuery')}
                </YakitSelect.Option>
                <YakitSelect.Option value="split_query">
                  {t('playground.KnowledgeBaseQA.splitQuery')}
                </YakitSelect.Option>
                <YakitSelect.Option value="hypothetical_answer_with_split">
                  {t('playground.KnowledgeBaseQA.hypotheticalAnswerWithSplit')}
                </YakitSelect.Option>
              </YakitSelect>
            </div>
          </div>
