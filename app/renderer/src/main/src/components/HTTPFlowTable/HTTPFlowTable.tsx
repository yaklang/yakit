...
export const HTTPFlowTable = React.memo<HTTPFlowTableProp>((props) => {
    ...
    const [calcTotal, setCalcTotal] = useState<boolean>(!initialIsMITM)
    ...
    const getAddDataByGrpc = useMemoizedFn((query) => {
        if (!isLoop) return
        if (isMITMPage && !calcTotal) return
