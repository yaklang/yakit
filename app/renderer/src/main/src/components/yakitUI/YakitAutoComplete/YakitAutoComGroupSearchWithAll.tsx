...
export const YakitAutoComGroupSearchWithAll = React.forwardRef<YakitAutoCompleteRefProps, YakitAutoCompleteProps>(
  (props, ref) => {
    ...
    const delCatchOptionItem = useMemoizedFn((e: React.MouseEvent<Element, MouseEvent>, item: YakitOptionTypeProps) => {
      e.stopPropagation()
      if (!cacheHistoryDataKey) return
      onSetRemoteValuesBase({
        cacheHistoryDataKey,
    ...
    const options = useMemo(() => {
      ...
      const matched = sourceFlatOptions.filter((opt) => optionMatchesInput(dropdownSearchText, opt, filterOptionProp))
          ...
          : [
              {
                ...
                label: <span className={styles['group-empty-placeholder']}>{searchResultEmptyLabel}</span>,
                disabled: true,
              },
            ]
