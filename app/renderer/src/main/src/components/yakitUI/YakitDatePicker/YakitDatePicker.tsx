  ...
  return (
    ...
    >
      <DatePicker
        ...
        disabledDate={
          disabledDate
            ? (current, info) => disabledDate(current ? moment(current.valueOf()) : moment(), info)
            : undefined
        }
        ...
        Object.entries(ranges).map(([key, item]) => {
          if (typeof item === 'function') {
            return [
              ...
              () => {
                const result = item()
                return result ? [toDayjs(result[0] as Moment | null), toDayjs(result[1] as Moment | null)] : result
              },
            ]
          }
          return [key, item ? [toDayjs((item as Moment[])[0]), toDayjs((item as Moment[])[1])] : item]
        }),
      )
    : undefined
