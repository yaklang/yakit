...
import { yakitApp, yakitEngine, yakitSystem, yakitWindowControls } from '@/services/electronBridge'
...
export const InstallEngine: React.FC<InstallEngineProps> = React.memo((props) => {
  ...
  useEffect(() => {
    yakitSystem.fetchSystemAndArch().then((e: string) => setPlatformArch(e))
  ...
  const onClose = useMemoizedFn(() => {
    ...
    yakitEngine.cancelDownloadYakEngineVersion(latestVersionRef.current)
    setTimeout(() => {
      setInstall(false)
  ...
  const downloadEngine = useMemoizedFn(async () => {
    try {
      const res = await yakitEngine.verifyYakEngineVersion(latestVersionRef.current)
      if (res === true) {
        yaklangUpdate()
      } else {
        realDownloadYaklang()
  ...
  const realDownloadYaklang = () => {
    setInstall(true)
    yakitEngine
      .downloadLatestYak(`${latestVersionRef.current}`)
      .then(() => {
        if (isBreakDownload.current) return
  ...
  const yaklangUpdate = () => {
    ...
    /** 安装yaklang引擎 */
    yakitEngine
      .installYakEngine(`${latestVersionRef.current}`)
      .then(() => {
        if (isBreakDownload.current) return
                ...
                {install ? (
                  <div className={classNames(styles['hint-right-download'], 'yakit-progress-wrapper')}>
                    ...
                    <div className={styles['download-btn']}>
                      <YakitButton loading={cancelLoading} size="max" type="outline2" onClick={onClose}>
                        取消
                      </YakitButton>
                    </div>
                  </div>
                    ...
                    <div className={styles['hint-right-btn']}>
                      <div>
                        ...
                        {haveBuildInEngine && (
                          <YakitPopconfirm
                            placement="top"
                            title={'网络安装需要公网环境，请知晓，请优先使用内置引擎（初始化引擎）'}
                      ...
                      <div className={styles['btn-group-wrapper']}>
                        <YakitButton size="max" type="outline2" onClick={() => yakitWindowControls.operate('close')}>
                          取消
                        </YakitButton>
