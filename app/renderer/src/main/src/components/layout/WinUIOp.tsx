...
export const WinUIOp: React.FC<WinUIOpProp> = React.memo((props) => {
    ...
    useEffect(() => {
        ipcRenderer.on("set-is-max", async (flag: boolean) => setIsMax(flag))

        return () => ipcRenderer.removeAllListeners("set-is-max")
    }, [])
    ...
    return (
        ...
        >
            <div className={styles["win-ui-op-body"]}>
                ...
                <div className={styles["op-btn"]} onClick={() => operate("full")}>
                    {isMax ? (
