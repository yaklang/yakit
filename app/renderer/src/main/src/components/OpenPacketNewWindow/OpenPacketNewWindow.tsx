...
const OpenPacketNewWindow: React.FC<OpenPacketNewWindowProps> = (props) => {
  ...
  const reqEditor = () => {
    return <NewHTTPPacketEditor originValue={reqOriginValue} readOnly={true} noMinimap={true} onlyBasicMenu={true} />
  }
  const resEditor = () => {
    return (
      <NewHTTPPacketEditor
  ...
  if (data.showParentPacketCom) {
    ...
    if (reqOriginValue && resOriginValue) {
      ...
      // 请求 响应都没有 默认展示个空编辑器
      return <NewHTTPPacketEditor originValue={''} readOnly={true} noMinimap={true} onlyBasicMenu={true} />
    }
  }
}
