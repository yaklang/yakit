...
export const ConfigNetworkPage: React.FC<ConfigNetworkPageProp> = (props) => {
  ...
  useEffect(() => {
    getLocalValue(LocalGVS.IsDeletePrivatePluginsOnLogout).then((v) => {
      setIsDelPrivatePlugin(v === true)
    })
  }, [])
  ...
  const onCertificate = useMemoizedFn((file: any) => {
    ...
    ipc
      .invoke('local', 'fetch-certificate-content', file.path)
      .then((res) => {
        currentIndex.current += 1

        // 验证证书是否需要密码
        ipc
          .invoke('grpc', 'ValidP12PassWord', {
            Pkcs12Bytes: res,
  ...
  const ipcSubmit = useMemoizedFn((params: GlobalNetworkConfig, isNtml?: boolean) => {
    ...
    ipc
      .invoke('grpc', 'SetGlobalNetworkConfig', realParams)
      .then(() => {
  ...
  const onConfirmCloseConfirmEnabled = useMemoizedFn(async (value: boolean) => {
    try {
      await ipc.invoke('local', 'set-extra-cache', { key: 'windows-close-flag', value: value })
      await ipc.invoke('local', 'manual-write-file', 'extraCache')
    } catch {}
  })
  ...
  useEffect(() => {
    ...
    ipc
      .invoke('local', 'fetch-extra-cache', 'windows-close-flag')
      .then((flag) => {
  ...
  useEffect(() => {
    getRemoteValue(RemoteGV.GlobalChromePath).then((setting) => {
      if (!setting) {
        ipc.invoke('local', 'GetChromePath', {}).then((chromePath) => {
                  ...
                  <Space>
                    ...
                    <YakitPopconfirm
                      ...
                      onConfirm={() => {
                        ...
                        ipc.invoke('grpc', 'ResetGlobalNetworkConfig', {}).then(() => {
                          cerFormRef.current?.resetFields()
                          update()
