...
try {
  ...
  const saveFuzzerSequenceCache = debounce(
    (selectedState) => {
      ...
      setRemoteProjectValue(FuzzerRemoteGV.FuzzerSequenceCache, JSON.stringify(sequenceCache)).catch((error) => {})
    },
    500,
    { leading: true },
  )
} catch (error) {
  yakitNotify('error', tOriginal('fuzzerSequence.cacheFailed', { error: String(error) }))
}
