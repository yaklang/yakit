...
import { ipc, type StreamTask, type GrpcInput } from '@/services/ipc'
...
let duplexSession: { controller: AbortController; opening?: Promise<StreamTask<'DuplexConnection'>> } | undefined
...
export const setFlowCommittedMode = async (mode: MITMFlowCommittedMode) => {
  ...
  await writeDuplex(flowCommittedSubscriptionRequest(mode))
}
...
const writeDuplex = async (params: GrpcInput<'DuplexConnection'>) => {
  const session = duplexSession
  if (!session?.opening) return
  const task = await session.opening
  if (duplexSession !== session || session.controller.signal.aborted) return
...
export const startupDuplexConn = () => {
  ...
  duplexSession?.controller.abort()
  const session = {
    controller: new AbortController(),
    opening: undefined as Promise<StreamTask<'DuplexConnection'>> | undefined,
  }
  ...
  const onError = (error: unknown) => {
    if (duplexSession !== session || session.controller.signal.aborted) return
    duplexSession = undefined
    duplexStarted = false
    flowCommittedRefreshScheduler.cancel()
    updateServerPushStatus(false)
    console.error(error)
  }
  session.opening = ipc.openStream('grpc', 'DuplexConnection', flowCommittedSubscriptionRequest(flowCommittedMode), {
    token: id,
...
export const sendDuplexConn = (params: DuplexConnectionProps) => {
  void writeDuplex(params).catch((error) => console.error(error))
}
