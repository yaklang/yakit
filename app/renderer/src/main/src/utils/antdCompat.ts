...
export function normalizeDropdownProps<T extends Record<string, unknown>>(props: T = {} as T) {
  ...
  return {
    ...
    rootClassName: [rootClassName, overlayClassName].filter(Boolean).join(' ') || undefined,
  }
}
...
/**
 * 把 Collapse.Panel / YakitPanel 子节点转成 antd 5 items。
 * 如果任一子节点缺少 header（包装组件如 MatchersPanel，header 在其内部渲染而非作为 prop 传入），
 * 返回 undefined 表示无法转换，调用方应回退到 children 路径让 antd 内部处理。
 */
