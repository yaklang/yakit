...
export const useProxy = () => {
  ...
  const comparePointUrl = useMemoizedFn((findKey) => {
    ...
    if (!!findPoint) {
      ...
      if (UserName && Password) {
        ...
        if (protocolMatch) {
          const protocol = protocolMatch[1] // http:// 或 https://
          const domain = protocolMatch[2]   // mi.com
  ...
  const checkProxyEndpoints = useMemoizedFn((proxy?: string[]) => {
    ...
    const newEndpoints = proxy.filter((item) =>
      proxyConfig.Endpoints.every(({ Id, Url }) => ![Id, Url].includes(item))
    )
