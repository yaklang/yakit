...
import { yakitShell } from '@/utils/electronBridge'
...
export const HelpDoc: React.FC<HelpDocProps> = React.memo((props) => {
  ...
  const menuSelect = useMemoizedFn((type: string) => {
    ...
    switch (type) {
      case 'report_bug':
        yakitShell.openUrl(`https://github.com/yaklang/yakit/issues/new?template=bug_report.yml`)
        return
      case 'feature_request':
        yakitShell.openUrl(`https://github.com/yaklang/yakit/issues/new?template=feature_request.yml`)
        return
      case 'official_website':
        yakitShell.openUrl(WebsiteGV.YakHelpDocAddress)
        return
      default:
        return
    }
