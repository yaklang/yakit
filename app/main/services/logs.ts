...
import { renderLogOutputFile } from '../logFile'
interface LogEntry {
  name: string
  title: string
  content?: unknown
 ...
 */
function pushLog(name: string, log: string) {
 ...
 */
function simpleLog({ name, title, content, time = new Date().toLocaleString(), withNewLine = true }: LogEntry) {
 ...
 */
function addLog({ name, title, content, time = new Date().toLocaleString(), status }: LogEntry) {
  if (!logs.has(name)) logs.set(name, [])
  const logList = logs.get(name)!
 ...
 * @returns {string[]} 日志数组
 */
function getLastLogs(name: string, count = 10) {
  const logList = logs.get(name) || []
 ...
 * @param {string} name - 日志名
 */
function clearLogs(name?: string) {
  if (name === undefined) logs.clear()
  else logs.delete(name)
}

function sortLogs(list: string[]) {
...
export = {
  ...
  getLastLogs,
  register: () => {
    registerMainMethod('add-log', (params) => {
      addLog(params)
    })
    registerMainMethod('add-simple-log', (params) => {
      simpleLog(params)
    })
  },
}
