...
type ProcessOptions = { all?: boolean }

const TEN_MEGABYTES = 1000 * 1000 * 10
const execFile = util.promisify(childProcess.execFile)

const windows = async (): Promise<ProcessInfo[]> => {
  ...
  const binPath = resourcePath('libs', 'extVendor', bin).replace('app.asar/', 'app.asar.unpacked/')
...
const nonWindowsMultipleCalls = async (options: ProcessOptions = {}): Promise<ProcessInfo[]> => {
...
const nonWindowsSingleCall = async (options: ProcessOptions = {}): Promise<ProcessInfo[]> => {
  let stdout = ''
  try {
    const flags = options.all === false ? 'wwxo' : 'awwxo'
    // TODO: Use the promise version of `execFile` when https://github.com/nodejs/node/issues/28244 is fixed.
    const [psPid, stdoutRaw] = await new Promise<[number | undefined, string]>((resolve, reject) => {
      const child = childProcess.exec(
        `ps ${flags} ${psFields} | grep 'yak[a-zA-Z_]*\\sgrpc' | grep -v grep`,
        { maxBuffer: TEN_MEGABYTES },
        (error, stdout) => {
          if (error === null) {
...
const nonWindows = async (options: ProcessOptions = {}): Promise<ProcessInfo[]> => {
  try {
...
export = process.platform === 'win32' ? windows : nonWindows
