...
import { getEngineLogDir, getRenderLogDir, getPrintLogDir } from './filePath'
 ...
 */
const clearFolder = (folderPath: string, length: number) => {
  try {
...
/** 打开文件并获取文件句柄 */
const getLogFileHandle = (folderPath: string, fileName: string, info: string) => {
...
/** 通过文件句柄写入内容 */
const writeLogFileContent = (fileHandle: number | null, content: string, info: string) => {
  if (fileHandle) {
...
// #region 引擎日志
/** 引擎日志文件句柄 */
let engineLogHandle: number | null = null
...
const engineContent: string[] = []
let engineTime: ReturnType<typeof setTimeout> | null = null
/** 引擎日志-往文件里输出信息 */
const engineLogOutputFile = (message: unknown) => {
  const { info } = TypeToLogBasicInfo.engine || {}
 ...
 */
const engineLogOutputUI = (win: BrowserWindow | null, message: unknown, isTitle?: boolean) => {
  const channle = !!isTitle ? 'live-engine-log' : 'live-engine-stdio'
  try {
    if (win) {
      sendEvent(win.webContents, channle, `${message}\n`)
    }
  } catch (error) {
 ...
 * @param {Boolean} isTitle 是否带标题([IFNO] ...)
 */
const engineLogOutputFileAndUI = (win: BrowserWindow | null, message: unknown, isTitle?: boolean) => {
...
// #region 渲染端日志
/** 渲染端日志文件句柄 */
let renderLogHandle: number | null = null
...
let renderTime: ReturnType<typeof setTimeout> | null = null
/** 渲染端日志-往文件里输出信息 */
const renderLogOutputFile = (message: unknown) => {
  const { info } = TypeToLogBasicInfo.render || {}
...
// #region 主动输出信息日志
/** 信息日志文件句柄 */
let printLogHandle: number | null = null
...
const printContent: string[] = []
let printTime: ReturnType<typeof setTimeout> | null = null
/** 输出信息日志-往文件里输出信息 */
const printLogOutputFile = (message: unknown) => {
  const { info } = TypeToLogBasicInfo.print || {}
