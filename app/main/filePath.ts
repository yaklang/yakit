...
const getConfig = (): AppConfig => {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) throw new Error('invalid app config')
    const config = { ...DEFAULT_CONFIG, ...parsed }
    // Keep extension keys, but do not pass malformed paths/settings into Node APIs.
    return {
      ...config,
      YAKIT_HOME: typeof config.YAKIT_HOME === 'string' ? config.YAKIT_HOME : DEFAULT_CONFIG.YAKIT_HOME,
      workspaceHistory: Array.isArray(config.workspaceHistory)
        ? config.workspaceHistory.filter((item): item is string => typeof item === 'string')
        : [],
      autoStart: typeof config.autoStart === 'boolean' ? config.autoStart : DEFAULT_CONFIG.autoStart,
      softTheme: typeof config.softTheme === 'string' ? config.softTheme : DEFAULT_CONFIG.softTheme,
      softLange: typeof config.softLange === 'string' ? config.softLange : DEFAULT_CONFIG.softLange,
      yakitMode: typeof config.yakitMode === 'string' ? config.yakitMode : DEFAULT_CONFIG.yakitMode,
    }
  } catch (e) {
...
const getYakitHome = () => {
  try {
    ...
    let homePath: string | undefined = config.YAKIT_HOME
...
const loadExtraFilePath = (s: string) => {
  if (!app.isPackaged) {
