...
function createYakitEngineLinkWindow(obj) {
    if (yakitEngineLinkWin) {
        ...
        if (win.isVisible()) {
            win.webContents.send("modal-mask:show")
        } else {
            win.webContents.send("modal-mask:hide")
        }
        makeWindowTopOnlyInApp(yakitEngineLinkWin)
    ...
    yakitEngineLinkWin.on("minimize", () => {
        if (win) {
            win.minimize()
            safeSend(win, "minimize-windows-renderer")
        }
    })
...
// 窗口是否置顶
function makeWindowTopOnlyInApp(win) {
...
function registerGlobalIPC() {
    ...
    ipcMain.handle("trigger-reload", () => {
        ...
        win.hide()
        yakitEngineLinkWin.webContents.reload()
    ...
    ipcMain.handle("trigger-reload-cache", () => {
        ...
        win.hide()
        yakitEngineLinkWin.webContents.reloadIgnoringCache()
    ...
    try {
        ...
        console.log("[yakitEngineLinkWin] registerNewIPC completed")
    } catch (err) {
        console.error("[yakitEngineLinkWin] registerNewIPC error:", err)
    }
}
