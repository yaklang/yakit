...
function createEngineLinkWindow() {
  ...
  if (shouldOpenDevTools) engineLinkWin.webContents.openDevTools({ mode: 'detach' })
  ...
  engineLinkWin.webContents.on('did-finish-load', () => {
    const msg = `[engineLinkWin] did-finish-load, URL: ${engineLinkWin.webContents.getURL()}`
    console.log(msg)
    printLogOutputFile(msg)
  })
  ...
  engineLinkWin.webContents.on('did-fail-load', (event, errorCode, errorDescription, validatedURL) => {
    const msg = `[engineLinkWin] did-fail-load: ${errorCode} - ${errorDescription}, URL: ${validatedURL}`
    console.error(msg)
    printLogOutputFile(msg)
  })
...
function createWindow() {
  ...
  win.webContents.on('did-fail-load', (event, errorCode, errorDescription, validatedURL) => {
    const msg = `[mainWin] did-fail-load: ${errorCode} - ${errorDescription}, URL: ${validatedURL}`
    console.error(msg)
    printLogOutputFile(msg)
  })
