...
type SenderEvent = { senderFrame?: { url: string } | null; sender?: { getURL(): string } }
type UrlOptions = { allowedHosts?: readonly string[]; requireHttps?: boolean }
...
const getSenderUrl = (event: SenderEvent) => {
  return event?.senderFrame?.url || event?.sender?.getURL?.() || ''
}

const isTrustedAppSender = (event: SenderEvent) => {
...
const assertTrustedAppSender = (event: SenderEvent, action: string) => {
  if (!isTrustedAppSender(event)) {
    throw new Error(`untrusted IPC sender for ${action}`)
  }
}

const isAllowedHostname = (hostname: string, allowedHosts: readonly string[] = []) => {
...
const normalizeHttpUrl = (value: unknown, options: UrlOptions = {}) => {
...
const normalizeHttpBaseUrl = (value: unknown, options: UrlOptions = {}) => {
...
const normalizeRelativeApiPath = (value: unknown) => {
  if (typeof value !== 'string') {
...
const validateOpenPath = (targetPath: unknown, options: { allowBlockedExtensions?: boolean } = {}) => {
...
const normalizePid = (value: unknown) => {
