...
contextBridge.exposeInMainWorld('yakitBridge', {
  ...
  engine: {
    ...
    fetchBundledEngineBuildType: () => invokePrefixed('fetch-bundled-engine-build-type'),
    ...
    currentLocalEngine: () => invokePrefixed('local-engine-current'),
    stopLocalEngine: (id) => invokePrefixed('local-engine-stop', id),
    stopAllLocalEngines: () => invokePrefixed('local-engine-stop-all'),
    disconnectLocalEngine: () => invokePrefixed('local-engine-disconnect'),
