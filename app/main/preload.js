...
process.on('loaded', function () {
  ...
  window.yakitBridge = {
    ...
    engine: {
      ...
      currentLocalEngine: () => invoke('local-engine-current'),
      stopLocalEngine: (id) => invoke('local-engine-stop', id),
      stopAllLocalEngines: () => invoke('local-engine-stop-all'),
      disconnectLocalEngine: () => invoke('local-engine-disconnect'),
