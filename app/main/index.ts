import { sendEvent } from './ipc/events'
import { registerRenderer, registerMainMethod } from './ipc/index'
import { appPath, preloadPath, rendererPath } from './paths'
import { app, BrowserWindow, dialog, nativeImage, globalShortcut, protocol, Menu } from 'electron'
import isDev from 'electron-is-dev'
import path from 'node:path'
import os from 'node:os'
import url from 'node:url'
import process from 'node:process'
const { configureE2EEnvironment } = require('./e2eEnvironment')

// This must run before localCache/filePath are loaded: both can resolve and
// create files below Electron's userData directory during module startup.
const e2eEnvironment = configureE2EEnvironment(app)

const { registerMainServices, registerLinkServices, getClient } = require('./bootstrap')
import { installCommunication } from './ipc/index'
const {
  initExtraLocalCache,
  getExtraLocalCacheValue,
  initLocalCache,
  setCloeseExtraLocalCache,
  setLocalCache,
} = require('./localCache')
const { asyncKillDynamicControl } = require('./services/control')
const { getWindowStatePath, getConfig } = require('./filePath')
const Screenshots = require('./services/screenshots')
import windowStateKeeper from 'electron-window-state'
const { MenuTemplate } = require('./menu')
const {
  renderLogOutputFile,
  getAllLogHandles,
  closeAllLogHandles,
  initAllLogFolders,
  printLogOutputFile,
} = require('./logFile')

installCommunication(getClient, (error) => printLogOutputFile(error instanceof Error ? error.stack : String(error)))

const BLOCKED_CHROMIUM_DEBUG_SWITCHES = ['remote-debugging-port', 'remote-debugging-address', 'remote-debugging-pipe']
const BLOCKED_NODE_DEBUG_ARG_PREFIXES = ['--inspect', '--inspect-brk', '--inspect-port']
const MITM_DEBUG_HOOKS_ARGUMENT = '--yakit-mitm-debug-hooks=1'
const mitmDebugHooksEnabled = !app.isPackaged && (isDev || e2eEnvironment.enabled)

const getForbiddenStartupDebugFlags = () => {
  const detected: string[] = []

  BLOCKED_CHROMIUM_DEBUG_SWITCHES.forEach((theSwitch) => {
    if (!app.commandLine.hasSwitch(theSwitch)) {
      return
    }

    const value = app.commandLine.getSwitchValue(theSwitch)
    detected.push(value ? `--${theSwitch}=${value}` : `--${theSwitch}`)
  })
  ;[...process.execArgv, ...process.argv].forEach((arg) => {
    if (typeof arg !== 'string') {
      return
    }

    if (
      BLOCKED_NODE_DEBUG_ARG_PREFIXES.some((prefix) => {
        return arg === prefix || arg.startsWith(`${prefix}=`)
      })
    ) {
      detected.push(arg)
    }
  })

  return [...new Set(detected)]
}

const removeForbiddenChromiumDebugSwitches = () => {
  BLOCKED_CHROMIUM_DEBUG_SWITCHES.forEach((theSwitch) => {
    try {
      app.commandLine.removeSwitch(theSwitch)
    } catch (error) {}
  })
}

const forbiddenStartupDebugFlags = app.isPackaged ? getForbiddenStartupDebugFlags() : []
const shouldAbortStartupForDebugFlags = app.isPackaged && forbiddenStartupDebugFlags.length > 0

if (shouldAbortStartupForDebugFlags) {
  removeForbiddenChromiumDebugSwitches()
  const blockedMessage =
    `[Main] blocked packaged startup due to forbidden debug flags: ` + `${forbiddenStartupDebugFlags.join(', ')}`

  console.error(blockedMessage)
  try {
    printLogOutputFile(blockedMessage)
  } catch (error) {}

  app.once('ready', () => {
    try {
      dialog.showErrorBox('Unsupported startup parameters', 'Yakit rejected forbidden debug startup flags.')
    } catch (error) {}
    app.exit(1)
  })
}

/** 获取缓存数据-软件是否需要展示关闭二次确认弹框 */
const UICloseFlag = 'windows-close-flag'

/** 窗口对象 */
let win: BrowserWindow | null = null
let engineLinkWin: BrowserWindow | null = null

process.on('uncaughtException', (error) => {
  try {
    printLogOutputFile(`[Main] index / uncaughtException => ${error?.message || JSON.stringify(error)}`)
  } catch (error) {}
})

// 性能优化：https://juejin.cn/post/6844904029231775758

// ======= 全局标志（每个窗口保证只注册一次） =======
let ipcRegistered = false

/**
 * ---------------- 创建 yakitEngineLink 窗口 ----------------
 */
let readyEngineLinkShow = false
function createEngineLinkWindow() {
  const state = windowStateKeeper({
    defaultWidth: 900,
    defaultHeight: 600,
    path: getWindowStatePath(),
    file: 'link-window-state.json',
  })
  const hasPos = Number.isFinite(state.x) && Number.isFinite(state.y)

  const createdWindow = new BrowserWindow({
    x: hasPos ? state.x : undefined,
    y: hasPos ? state.y : undefined,
    width: 900,
    height: 600,
    minWidth: 900,
    minHeight: 600,
    frame: false,
    autoHideMenuBar: true,
    resizable: false,
    webPreferences: {
      preload: preloadPath('engine-link'),
      nodeIntegration: false,
      contextIsolation: true,
      sandbox: true,
    },
    titleBarStyle: 'hidden',
    show: true,
    skipTaskbar: false,
    fullscreenable: false,
    maximizable: false,
  })
  engineLinkWin = createdWindow
  registerRenderer(createdWindow.webContents, 'link')

  if (!hasPos) createdWindow.center()
  if (isDev) createdWindow.loadURL('http://127.0.0.1:5173')
  else createdWindow.loadFile(rendererPath('link'))

  createdWindow.webContents.setWindowOpenHandler(() => ({ action: 'deny' }))

  if (isDev) createdWindow.webContents.openDevTools({ mode: 'detach' })

  createdWindow.setMenu(null)
  createdWindow.setMenuBarVisibility(false)
  if (process.platform === 'darwin') createdWindow.setWindowButtonVisibility(false)

  createdWindow.webContents.on('will-navigate', (e) => e.preventDefault())

  createdWindow.on('show', () => {
    printLogOutputFile(`[engineLinkWin] show event triggered`)
    state.manage(createdWindow)
  })

  createdWindow.once('ready-to-show', () => {
    readyEngineLinkShow = true
    printLogOutputFile(
      `[engineLinkWin] ready-to-show, isVisible: ${createdWindow.isVisible()}, isDestroyed: ${createdWindow.isDestroyed()}`,
    )
  })

  createdWindow.webContents.on('did-finish-load', () => {
    printLogOutputFile(`[engineLinkWin] did-finish-load, URL: ${createdWindow.webContents.getURL()}`)
  })

  createdWindow.webContents.on('did-frame-finish-load', (event, isMainFrame) => {
    if (isMainFrame) {
      printLogOutputFile(
        `[engineLinkWin] main frame finished load, isLoadingMainFrame: ${createdWindow.webContents.isLoadingMainFrame()}`,
      )
    }
  })

  createdWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription, validatedURL) => {
    printLogOutputFile(`[engineLinkWin] did-fail-load: ${errorCode} - ${errorDescription}, URL: ${validatedURL}`)
  })

  createdWindow.webContents.on('did-stop-loading', () => {
    printLogOutputFile('[engineLinkWin] did-stop-loading')
  })

  createdWindow.webContents.on('render-process-gone', (event, details) => {
    const snapshot = collectDevicePerformanceSnapshot()
    renderLogOutputFile(`----- engineLinkWin Render gone ------`)
    renderLogOutputFile(`reason: ${details.reason}, exitCode: ${details.exitCode}`)
    printLogOutputFile(`[engineLinkWin] render-process-gone snapshot: ${JSON.stringify(snapshot)}`)
    if (details.reason === 'crashed') setLocalCache('render-crash-screen', true)
    require('./services/logs').saveLogs()
  })

  createdWindow.on('close', (e) => {
    state.saveState(createdWindow)
    // WDIO owns the isolated Electron process lifecycle. Let its teardown
    // close the window directly instead of opening the user-facing dialog.
    if (e2eEnvironment.enabled) return

    e.preventDefault()
    if (createdWindow.isVisible()) {
      sendEvent(createdWindow.webContents, 'close-engineLinkWin-renderer')
    }
  })

  createdWindow.on('closed', () => {
    engineLinkWin = null
  })
}

/**
 * ---------------- 创建主窗口 ----------------
 */
let readyWinShow = false
function createWindow() {
  const minWidth = 900
  const minHeight = 650
  const defaultWidth = 1400
  const defaultHeight = 880
  const state = windowStateKeeper({
    defaultWidth: defaultWidth,
    defaultHeight: defaultHeight,
    path: getWindowStatePath(),
    file: 'yakit-window-state.json',
  })
  const width = Math.max(state.width ?? defaultWidth, minWidth)
  const height = Math.max(state.height ?? defaultHeight, minHeight)

  const createdWindow = new BrowserWindow({
    x: state.x,
    y: state.y,
    width: width,
    height: height,
    minWidth: minWidth,
    minHeight: minHeight,
    frame: false,
    autoHideMenuBar: true,
    webPreferences: {
      preload: preloadPath('main'),
      additionalArguments: mitmDebugHooksEnabled ? [MITM_DEBUG_HOOKS_ARGUMENT] : [],
      nodeIntegration: true,
      contextIsolation: false,
      sandbox: true,
      backgroundThrottling: false,
    },
    titleBarStyle: 'hidden',
    show: false,
    skipTaskbar: true,
  })
  win = createdWindow
  registerRenderer(createdWindow.webContents, 'main')

  if (isDev) createdWindow.loadURL('http://127.0.0.1:3000')
  else createdWindow.loadFile(rendererPath('main'))

  if (isDev) createdWindow.webContents.openDevTools({ mode: 'detach' })

  createdWindow.setMenu(null)
  createdWindow.setMenuBarVisibility(false)
  if (process.platform === 'darwin') createdWindow.setWindowButtonVisibility(false)

  createdWindow.webContents.setWindowOpenHandler(() => ({ action: 'deny' }))
  createdWindow.webContents.on('will-navigate', (e) => e.preventDefault())

  createdWindow.on('show', () => {
    printLogOutputFile(`[mainWin] show event triggered`)
    state.manage(createdWindow)
  })

  createdWindow.once('ready-to-show', () => {
    readyWinShow = true
    printLogOutputFile(
      `[mainWin] ready-to-show, isVisible: ${createdWindow.isVisible()}, isDestroyed: ${createdWindow.isDestroyed()}`,
    )
  })

  // F5 / reload 时清掉 ready 标记，避免刷新过程中误发，并让 markRenderOk 重新走一遍
  createdWindow.webContents.on('did-start-loading', () => {
    clearRenderMap(createdWindow)
  })

  createdWindow.webContents.on('did-finish-load', () => {
    printLogOutputFile(`[mainWin] did-finish-load, URL: ${createdWindow.webContents.getURL()}`)
  })

  createdWindow.webContents.on('did-frame-finish-load', (event, isMainFrame) => {
    if (isMainFrame) {
      printLogOutputFile(
        `[mainWin] main frame finished load, isLoadingMainFrame: ${createdWindow.webContents.isLoadingMainFrame()}`,
      )
    }
  })

  createdWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription, validatedURL) => {
    printLogOutputFile(`[mainWin] did-fail-load: ${errorCode} - ${errorDescription}, URL: ${validatedURL}`)
  })

  createdWindow.webContents.on('did-stop-loading', () => {
    printLogOutputFile(`[mainWin] did-stop-loading`)
  })

  createdWindow.webContents.on('render-process-gone', (event, details) => {
    const snapshot = collectDevicePerformanceSnapshot()
    renderLogOutputFile(`----- Render gone ------`)
    renderLogOutputFile(`reason: ${details.reason}, exitCode: ${details.exitCode}`)
    printLogOutputFile(`[mainWin] render-process-gone snapshot: ${JSON.stringify(snapshot)}`)
    if (details.reason === 'crashed') setLocalCache('render-crash-screen', true)
    require('./services/logs').saveLogs()
  })

  createdWindow.on('close', (e) => {
    const bounds = createdWindow.getBounds()
    if (bounds.width >= minWidth && bounds.height >= minHeight) {
      state.saveState(createdWindow)
    }
    // Test runs use a disposable profile and are terminated by the runner.
    // Product startup and close confirmation remain unchanged outside E2E.
    if (e2eEnvironment.enabled) return

    e.preventDefault()
    if (createdWindow.isVisible()) {
      sendEvent(createdWindow.webContents, 'close-windows-renderer')
    }
  })

  createdWindow.on('minimize', () => {
    sendEvent(createdWindow.webContents, 'refresh-token')
    sendEvent(createdWindow.webContents, 'minimize-windows-renderer')
  })

  createdWindow.on('maximize', () => {
    sendEvent(createdWindow.webContents, 'refresh-token')
  })

  createdWindow.on('closed', () => {
    win = null
  })
}

/**
 * ---------------- 通用方法 ----------------
 */
// 窗口render准备好再发送数据
const renderMap = new Map<number, boolean>()
const messageQueue = new Map<number, { channel: string; data: unknown }[]>()
// 缓存最近一次 engineLinkWin 推送过来的引擎 credential，供主窗口 F5 刷新后重推，避免白屏
let lastEngineLinkCredential: unknown = null
function safeSend(targetWin: BrowserWindow | null, channel: string, data: unknown) {
  if (!targetWin || targetWin.isDestroyed()) return

  const id = targetWin.id
  if (!renderMap.get(id)) {
    if (!messageQueue.has(id)) {
      messageQueue.set(id, [])
    }
    messageQueue.get(id)!.push({ channel, data })
    return
  }
  sendEvent(targetWin.webContents, channel, data)
}
// render真正渲染出来
function markRenderOk(curWin: BrowserWindow | null) {
  if (!curWin) return
  const id = curWin.id

  renderMap.set(id, true)

  const queue = messageQueue.get(id) || []

  queue.forEach(({ channel, data }) => {
    sendEvent(curWin.webContents, channel, data)
  })
  messageQueue.delete(id)
}
// 关闭、reload清理渲染map
function clearRenderMap(targetWin: BrowserWindow | null) {
  if (!targetWin) return
  renderMap.delete(targetWin.id)
  messageQueue.delete(targetWin.id)
}
// 窗口隐藏
function winHide(targetWin: BrowserWindow | null) {
  if (targetWin && !targetWin.isDestroyed()) {
    targetWin.hide()
    targetWin.setSkipTaskbar(true)
  }
}
// 窗口显示
function winShow(targetWin: BrowserWindow | null, readyShow: boolean) {
  if (targetWin && !targetWin.isDestroyed()) {
    let shown = false
    const show = () => {
      if (shown) return
      shown = true
      targetWin.show()
      targetWin.focus()
      targetWin.setSkipTaskbar(false)
    }
    // 确保窗口已经加载完成
    if (targetWin.webContents.isLoading()) {
      printLogOutputFile(`winShow Loading...`)
      if (readyShow) {
        printLogOutputFile(`winShow ready ok`)
        show()
      } else {
        targetWin.webContents.once('did-finish-load', () => {
          show()
        })
      }
    } else {
      printLogOutputFile(`winShow Loading ok`)
      show()
    }
  }
}
// 窗口关闭
function winClose(targetWin: BrowserWindow | null, removeEvent: boolean) {
  if (targetWin && !targetWin.isDestroyed()) {
    removeEvent && targetWin.removeAllListeners('close')
    targetWin.close()
    targetWin = null
  }
}
// 获取当前窗口
function getActiveWindow() {
  // 优先：当前聚焦窗口
  const focused = BrowserWindow.getFocusedWindow()
  if (focused && !focused.isDestroyed()) {
    return focused
  }

  // 次优：可见窗口
  if (engineLinkWin?.isVisible()) return engineLinkWin
  if (win?.isVisible()) return win

  return null
}

/**
 * ---------------- 设备 / 性能 ----------------
 */
// 基础系统信息（Node / OS）
function getBaseSystemInfo() {
  return {
    platform: process.platform,
    arch: process.arch,
    osType: os.type(),
    osRelease: os.release(),
    loadAvg: os.loadavg(),
    uptime: os.uptime(),
    cpuCount: os.cpus().length,
    cpuModel: os.cpus()[0]?.model,
    totalMemoryGB: (os.totalmem() / 1024 / 1024 / 1024).toFixed(2),
    freeMemoryGB: (os.freemem() / 1024 / 1024 / 1024).toFixed(2),
    nodeVersion: process.version,
    electronVersion: process.versions.electron,
    chromeVersion: process.versions.chrome,
    v8Version: process.versions.v8,
  }
}
// GPU 核心信息
function getGPUInfo() {
  return {
    gpuFeatureStatus: app.getGPUFeatureStatus(),
    gpuInfoBasic: app.getGPUInfo('basic'),
    gpuInfoComplete: app.getGPUInfo('complete'),
  }
}
// Chromium 命令行 GPU Flags
function getGPUFlags() {
  return {
    disableGPU: app.commandLine.hasSwitch('disable-gpu'),
    disableGPUCompositing: app.commandLine.hasSwitch('disable-gpu-compositing'),
    useAngle: app.commandLine.getSwitchValue('use-angle'),
    enableFeatures: app.commandLine.getSwitchValue('enable-features'),
    disableFeatures: app.commandLine.getSwitchValue('disable-features'),
  }
}
// Electron 进程内存 / 性能状态
async function getProcessMetrics() {
  const metrics = app.getAppMetrics()
  return metrics.map((m) => ({
    pid: m.pid,
    type: m.type,
    cpuPercent: m.cpu.percentCPUUsage,
    memoryMB: (m.memory.workingSetSize / 1024).toFixed(1),
  }))
}
// GPU Crash / Blacklist 判断
function analyzeGPUStatus(gpuFeatureStatus: Electron.GPUFeatureStatus) {
  const badFeatures = Object.entries(gpuFeatureStatus).filter(([, v]) => v !== 'enabled')

  return {
    hasIssue: badFeatures.length > 0,
    badFeatures,
  }
}
function collectDevicePerformanceSnapshot() {
  return {
    time: new Date().toISOString(),
    base: getBaseSystemInfo(),
    gpu: getGPUInfo(),
    gpuFlags: getGPUFlags(),
    processes: getProcessMetrics(),
  }
}

/**
 * ---------------- 注册 IPC，只执行一次 ----------------
 */
function registerGlobalIPC() {
  if (ipcRegistered) return
  ipcRegistered = true

  // ------------------- 刷新相关 -------------------
  /** 刷新缓存 */
  registerMainMethod('trigger-reload', () => {
    lastEngineLinkCredential = null
    clearRenderMap(engineLinkWin)
    clearRenderMap(win)
    win?.webContents.reload()
    winHide(win)
    engineLinkWin?.webContents.reload()
    setTimeout(() => {
      winShow(engineLinkWin, readyEngineLinkShow)
    }, 500)
    return
  })
  /** 强制清空刷新缓存 */
  registerMainMethod('trigger-reload-cache', () => {
    lastEngineLinkCredential = null
    clearRenderMap(engineLinkWin)
    clearRenderMap(win)
    win?.webContents.reloadIgnoringCache()
    winHide(win)
    engineLinkWin?.webContents.reloadIgnoringCache()
    setTimeout(() => {
      winShow(engineLinkWin, readyEngineLinkShow)
    }, 500)
    return
  })

  /** 设置主窗口缩放比例(类原生应用缩放) */
  registerMainMethod('set-main-window-zoom-factor', (factor) => {
    const nextFactor = Number(factor)
    if (!Number.isFinite(nextFactor)) return
    const normalizeFactor = Math.min(2, Math.max(0.5, nextFactor))
    win?.webContents?.setZoomFactor(normalizeFactor)
  })

  // ------------------- render已准备好 -------------------
  registerMainMethod(
    'engine-win-render-ok',
    () => {
      markRenderOk(engineLinkWin)
    },
    ['link'],
  )
  registerMainMethod('main-win-uilayout-render-ok', () => {
    if (!win) return
    // 队列里已有 credential 时只冲刷，避免与首次入队重复推送
    const hadPending = (messageQueue.get(win.id) || []).some((m) => m.channel === 'from-engineLinkWin')
    markRenderOk(win)
    // F5 后队列为空但仍有缓存时重推，避免白屏
    if (lastEngineLinkCredential && !hadPending) {
      safeSend(win, 'from-engineLinkWin', lastEngineLinkCredential)
    }
  })

  // ------------------- 窗口发送数据操作 -------------------
  // engineLink 完成操作
  registerMainMethod(
    'engineLinkWin-done',
    async (data) => {
      // 缓存 credential，供主窗口 F5 刷新后重推
      lastEngineLinkCredential = data
      winHide(engineLinkWin)
      winShow(win, readyWinShow)
      safeSend(win, 'from-engineLinkWin', data)
    },
    ['link'],
  )

  // win 完成操作
  registerMainMethod('yakitMainWin-done', async (data) => {
    // 回到 engineLink 流程，清空缓存，避免主窗误用旧 credential
    lastEngineLinkCredential = null
    winHide(win)
    winShow(engineLinkWin, readyEngineLinkShow)
    safeSend(engineLinkWin, 'from-win', data)
  })

  // win 远程控制通知 engineLink 更新 认证信息
  registerMainMethod('updateCredential', async (data) => {
    // 同步缓存，保证 F5 重推的是最新 credential
    if (data) lastEngineLinkCredential = data
    safeSend(engineLinkWin, 'from-win-updateCredential', data)
  })

  // ------------------- 软件重启逻辑 -------------------
  registerMainMethod(
    'relaunch',
    () => {
      lastEngineLinkCredential = null
      clearRenderMap(engineLinkWin)
      clearRenderMap(win)
      winClose(engineLinkWin, true)
      winClose(win, true)
      closeAllLogHandles()
      app.relaunch()
      app.exit(0)
    },
    ['main', 'link'],
  )

  // ------------------- 软件退出逻辑 -------------------
  registerMainMethod(
    'app-exit',
    async (params) => {
      if (!params || typeof params !== 'object') throw new Error('Invalid app-exit parameters')
      const { showCloseMessageBox, isIRify, isMemfit } = params as {
        showCloseMessageBox?: boolean
        isIRify?: boolean
        isMemfit?: boolean
      }
      const parentWindow = getActiveWindow()

      const exitCleanupOperation = () => {
        clearRenderMap(engineLinkWin)
        clearRenderMap(win)
        winClose(engineLinkWin, false)
        winClose(win, false)
        closeAllLogHandles()
        app.exit()
      }
      if (
        !e2eEnvironment.enabled &&
        getExtraLocalCacheValue(UICloseFlag) !== false &&
        showCloseMessageBox &&
        parentWindow
      ) {
        const showIcon = isIRify
          ? '../assets/irify-close.png'
          : isMemfit
            ? '../assets/memfit-close.png'
            : '../assets/yakit-close.png'

        return dialog
          .showMessageBox(parentWindow, {
            icon: nativeImage.createFromPath(appPath('app', 'main', showIcon)),
            type: 'none',
            title: '提示',
            defaultId: 0,
            cancelId: 3,
            message: '确定要关闭吗？',
            buttons: ['最小化', '直接退出'],
            checkboxLabel: '不再展示关闭二次确认？',
            checkboxChecked: false,
            noLink: true,
          })
          .then(async (res) => {
            await setCloeseExtraLocalCache(UICloseFlag, !res.checkboxChecked)
            await asyncKillDynamicControl()
            if (res.response === 0) {
              engineLinkWin?.minimize()
              win?.minimize()
            } else if (res.response === 1) {
              exitCleanupOperation()
            } else {
            }
          })
      } else {
        await asyncKillDynamicControl()
        exitCleanupOperation()
      }
    },
    ['main', 'link'],
  )

  // ------------------- 窗口通信注册 -------------------
  try {
    registerLinkServices(engineLinkWin)
    printLogOutputFile('[engineLinkWin] registerLinkServices completed')
  } catch (err) {
    printLogOutputFile(`[engineLinkWin] registerLinkServices error: ${err}}`)
  }
  try {
    registerMainServices(win)
    const auxWindowHandler = require('./services/auxWindows')
    auxWindowHandler.configureBroadcast({ engineLinkWin, safeSend })
    printLogOutputFile('[Main] registerMainServices completed')
  } catch (err) {
    printLogOutputFile(`[Main] registerMainServices error: ${err}}`)
  }
}

/**
 * set software menu
 */
const menu = Menu.buildFromTemplate(MenuTemplate)
Menu.setApplicationMenu(menu)

/**
 * ---------------- App Ready ----------------
 */
if (!shouldAbortStartupForDebugFlags) {
  app.whenReady().then(async () => {
    /**
     * init-log-folders:
     * 存在则检查文件数量是否超过10个，超过则只保留最近10个文件
     * 不存在则新建文件夹
     */
    initAllLogFolders()

    /** 获取缓存数据并储存于软件内 */
    initLocalCache()

    getAllLogHandles()

    /** 获取扩展缓存数据并储存于软件内(是否弹出关闭二次确认弹窗) */
    initExtraLocalCache()

    // 截图功能注册
    if (['darwin', 'win32'].includes(process.platform)) {
      const screenshots = new Screenshots({ singleWindow: true })
      registerMainMethod('activate-screenshot', async () => {
        await screenshots.startCapture()
        globalShortcut.register('esc', () => {
          screenshots.endCapture()
          globalShortcut.unregister('esc')
        })
      })
      globalShortcut.register('Control+Shift+q', () => {
        screenshots.endCapture()
        globalShortcut.unregister('esc')
      })

      screenshots.on('ok', () => {
        if (screenshots.$win?.isFocused()) {
          screenshots.endCapture()
          globalShortcut.unregister('esc')
        }
      })
      screenshots.on('cancel', () => {
        globalShortcut.unregister('esc')
      })
    }

    // 注册协议
    protocol.registerFileProtocol('atom', (request, callback) => {
      const filePath = url.fileURLToPath('file://' + request.url.slice('atom://'.length))
      callback(filePath)
    })

    // 收集 设备 / 性能
    const snapshot = collectDevicePerformanceSnapshot()
    const { hasIssue, badFeatures } = analyzeGPUStatus(snapshot.gpu.gpuFeatureStatus)
    if (hasIssue) {
      printLogOutputFile(`hasIssue: ${JSON.stringify(badFeatures)}`)
    }
    printLogOutputFile(`after whenReady snapshot: ${JSON.stringify(snapshot)}`)

    createEngineLinkWindow()
    createWindow()

    app.on('activate', function () {
      if (BrowserWindow.getAllWindows().length === 0) {
        createEngineLinkWindow()
      }
    })

    // IPC
    try {
      registerGlobalIPC()
    } catch (error) {}
  })
}

app.on('child-process-gone', async (_e, killed) => {
  const snapshot = collectDevicePerformanceSnapshot()
  printLogOutputFile(`child-process-gone snapshot: ${JSON.stringify(snapshot)}, killed: ${JSON.stringify(killed)}`)
})

/**
 * ---------------- 全局退出逻辑 ----------------
 */
// 这个退出压根执行不到 win.on("close") 阻止了默认行为
app.on('window-all-closed', () => {
  app.quit()
})
