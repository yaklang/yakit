...
- **实际行为**：启动页、主进程和主界面二次校验共用开启策略；连接引擎后复验缓存授权，无有效授权时展示授权输入页，通过后进入数字员工。原授权协议、缓存和其他产品流程不变。
- **验证**：主进程授权服务、启动页和主界面授权入口共 3 个测试文件、12 项测试通过；直接加载主进程模块确认策略返回 `true`，JavaScript 语法检查通过。
- **生效范围**：源码运行需重启 Electron，安装包需基于当前源码重新构建。本次未重新打包，未完成实际 Electron 与授权签发服务联调。

## 2026-09-08

### 后端联调期间临时关闭 AI SenSo 客户端授权

- **统一开关**：`app/main/memfitLicense.js` 的 `MEMFIT_LICENSE_REQUIRED` 临时设为 `false`；主进程、启动页和主界面二次校验共用此策略，两个渲染器通过只读 IPC 获取。
- **实际行为**：连接引擎后直接进入数字员工，不展示授权输入，不调用启动授权校验，不写入虚假授权或修改缓存；其他产品的原授权流程保持原样。
- **生效范围**：当前源码运行和基于此提交重新构建的 AI SenSo 安装包；现有运行中的 Electron 需要重启，旧安装包需要重新构建。引擎自身的能力授权不在客户端改动范围内。
- **恢复方式**：将同一常量改回 `true`，重新构建并重启客户端；原授权验证服务、sender 校验和主窗口延迟加载机制均保留。
- **默认智能体交付文档**：新增 `docs/DEFAULT_AGENT_ASSIGNMENT_BACKEND.md`，列出八个角色、31 项默认映射以及后端初始化、迁移和验收要求。
- **验证**：主进程授权服务、启动页和主界面授权入口共 3 个测试文件、12 项测试通过，覆盖关闭／恢复授权、策略读取失败和其他产品保持原流程；主渲染 TypeScript、启动页 Memfit 生产构建、主进程语法及格式检查通过。本次没有重新打安装包，也没有在实际 Electron 窗口完成联调复测。
