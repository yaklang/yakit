...
- **2026-09-14 Windows 快捷方式修复**：`build/yakit_build.nsh` 安装/卸载统一使用打包元数据中的程序名，修正桌面快捷方式和安装完成后启动旧 `AI SenPike.exe` 的问题；已有旧快捷方式在新入口创建成功后迁移，并兼容旧卸载器先删除入口的升级流程。历史安装路径键保留兼容，卸载不再被该键重定向。11 个隔离 NSIS 运行场景及安装器编译通过；用户侧需重新打包安装，`release/shortcut-validation/` 仅为复用旧解包资源的编译验证产物。
- **2026-09-14 已恢复客户端启动授权，取代 2026-09-08 联调临时关闭状态**：按用户要求将 `app/main/memfitLicense.js` 的唯一开关恢复为 `MEMFIT_LICENSE_REQUIRED = true`；启动页、主进程和主界面二次校验统一读取。连接引擎后先复验缓存授权，无有效授权时展示授权输入页，通过后进入数字员工。作用于开发运行及基于当前代码重新构建的安装包；需重启 Electron。保留真实授权服务与缓存，不改变其他产品，也不改变引擎端的能力授权。
...
- 数字员工是固定业务角色，一个角色可以关联零个或多个 AI Forge 智能体。
- 用户在聊天欢迎页选择智能体后，输入框自动显示该智能体对应的原版 `forge` mention 标签。
- 用户仍可通过 `@` 或原有选择器继续添加一个或多个技能、工具、知识库标签。
- 发送后进入详情页，详情输入框会继续保留/恢复当前员工默认技能标签。
- 切换员工角色会清空智能体并回到选择状态；切换智能体时按 `selectionVersion` 重建输入编辑器，不残留旧智能体锁定标签。
- 请求层仍有智能体技能兜底和去重：即使界面标签初始化异常，选中智能体也会随消息发送；相同技能不会重复附加。
...
`pages/digitalEmployee/config.ts` 只定义固定八个角色的 ID、名称、职责、头像和主题色。AI Forge 是角色下可动态增减的智能体，通过 `roleAssignment.ts` 读写内部 Tag `senso-role:<role-id>` 建立唯一归属。`DigitalEmployeeContext.tsx` 分别维护 `selectedEmployee`（角色）和 `selectedAgent`（智能体），不再把 Forge 转换成员工，也不再依赖返回顺序。
...
4. `applyDigitalEmployeeSkillToInputEvent` 把当前选中智能体作为 `aiforge` 资源兜底加入并去重。
5. 首次启动参数仍通过 `applyForgeNameToStartParams` 写入真实 `ForgeName`。

因此，正确理解是：数字员工负责角色分类和工作区体验，真正执行对话的是用户在该角色下选择的 AI Forge。用户仍可继续叠加多个原版标签和输入内容一起发送。

### 2.2 默认智能体标签

- `resolver.ts` 中的 `getDigitalEmployeeDefaultMention` 把当前 `selectedAgent` 转换成原版 `forge` mention。
- 该标签使用真实 Forge Id、显示名，并设置 `lock: true`，界面不显示删除按钮。
- `AIChatTextarea` 新增 `defaultMentions`：编辑器初始化、清空、会话切换后都会恢复缺失的默认标签。
- 欢迎页与详情页以 `selectionVersion` 作为输入组件 key；智能体切换会创建干净的新输入实例，避免旧锁定标签与新标签叠加。
- 插入 mention 的底层命令只在光标前确实是 `@` 时删除触发字符，程序化插入不会误删用户文本。
- 欢迎页与对话详情页使用同一个输入组件和同一套 mention 提取逻辑。

不要再额外制作一套“看起来像标签”的普通 DOM；默认智能体必须继续走 Milkdown mention 数据链路。
...
- 选择智能体后，其默认技能在输入框内显示为原版标签；`@` 添加的其他标签仍可共存。
- 首次发送和详情页后续发送都会携带选中智能体，并对重复资源去重。
