...
1. 可见的 Link 或 Main 发生进程异常退出、主页面加载失败或无响应时，原生对话框提供“恢复界面”“回到连接页”“导出诊断”“退出软件”。隐藏窗口只记录故障，显示后再提示。打包后的生产环境和 E2E 测试会自动弹框；开发环境为避免 Vite 编译慢被误判为崩溃，改为静默模式，即自动记录故障但不主动弹框。
