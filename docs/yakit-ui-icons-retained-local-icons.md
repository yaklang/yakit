# @yakit-libs/yakit-ui-icons 0.2.1 历史待补充与改造清单

> 历史审查基线：`@yakit-libs/yakit-ui-icons@0.2.1`。当前迁移已锁定 `0.2.7`，且 `0.2.3` 已提供公开的 `oldicon/*` 独立 ESM。`0.2.7` 已将节点编号图标名替换为英文语义名（例如表中的 `FigmaIcon2017756Outlined` 现为 `ExportOutlined`），并提供 `IntentionOutlined`，AI 流消息已移除对应本地 SVG。下文的导出数量、缺口和候选列表是 0.2.1 时点的快照，不代表当前代码的本地图标残留。

## 1. 结论摘要

- 审查时 `0.2.1` 是 npm `latest`；包内共有 outline 350、solid 255、colorful 296，共 901 个导出。
...
原 95 个高相似候选中的 19 个零消费者实现已直接删除；剩余 76 项在当时仍需要产品消费者级视觉/API 复核。它们不是默认的包改造需求；本文保留已审计列表，当前状态应以代码和锁定的 0.2.7 包为准。
