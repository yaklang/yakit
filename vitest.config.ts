...
export default defineConfig({
  plugins: [
    {
      name: 'stub-css-modules-for-unit-tests',
      enforce: 'pre',
      resolveId(id) {
        if (!id.endsWith('.module.scss')) return null
        return `\0style-module:${id}.js`
      },
      load(id) {
        if (!id.startsWith('\0style-module:')) return null
        return `export default new Proxy({}, { get: (_target, property) => String(property) })`
      },
    },
    {
